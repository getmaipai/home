...
async function withRole(
  ...
  // stops being a bare-/select profile (requiresCredential()), so only
  // attach one, and sign in via verify-secret instead, for the roles
  // that now need it. child/teen/guest are untouched.
