 ...
 * Never touches the live `data/hub.db`: that swap belongs to
 * stageRestore/applyPendingRestore below, which are the only callers
...
// database handle; see that file's header). Re-exported here so callers
// still find the whole backup story in one place.
export {
  stageRestore,
  pendingRestore,
