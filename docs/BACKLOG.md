...
- [x] **ENGINE-PORT-01: the hub never kills an engine it did not spawn** (S, Sonnet, 2026-09-23; dev.md "The \"generation_failed\" outage on the new path was a killed engine"). Objective: `sidecars.ts`'s `freePort(port)` SIGKILLs every process whose command line carries `--port <n>` before every spawn, by port match and silently, so any second process that starts the hub's supervisors with the production defaults (a headless repro, a bench outside `tests/preload.ts`) kills the household's chat, text-understanding and embed engines, and the two hubs then ping-pong (74 SIGKILL lines, three "did not come back"). Change: an orphan is a pid the hub's own pid record names from a previous instance of this data directory (write the spawned engines' pids beside `data/local-app/pid`); `freePort` kills only those; a live holder that answers `/health` and is not in the record is a reported condition on the health list and the log ("port 8788 is held by pid N, not ours: set MAIPAI_LLAMA_SERVER_PORT, MAIPAI_BACKGROUND_PORT and MAIPAI_EMBED_PORT") and the spawn is refused, never a kill; every kill `freePort` does make is logged with the pid and the reason. Files: `sidecars.ts` (`freePort`, `findPidsMatching`), the supervisors' spawn paths, `sidecars.test.ts`. Tests in these words: a fake holder of the port that is not in the pid record is never signalled and the spawn returns the reported condition; a pid the record names from a dead instance is killed and logged; the health list carries the condition. Acceptance: two hubs started on the same ports from two data directories leave the first one's engines alive. Out of scope: the repro scripts' own isolation (they must set the port variables; SERVICES.md gets the one line). Exit: `bash scripts/check.sh`. Landed a1d363fb: the ownership record lives at `data/local-app/engine-pids.json` (beside the hub's own pid file), keyed by port; a port with no record at all falls back to the old kill-on-sight behavior (a real regression found in review otherwise - an upgrading install with a genuine crash orphan and no history yet would stay stuck blocked forever), so the protection is complete only once a port has been successfully spawned onto since this landed. The health-list surfacing is a new `"blocked"` `EngineHealthKind`, wired through all four engine roles (chat, background, embed, voice/tts) via `GET /api/health` and a HealthSection.tsx badge, not a Repairs issue. `data/local-app/pid` itself (the hub's own pid) is unchanged; `data/local-app/engine-pids.json` is a sibling record, not a shared file. Out of scope's own `docs/SERVICES.md` line (a different repo, `getmaipai/.github`) was not added this pass - flagged for the coordinator.

- [x] **GENFAIL-01: a failed generation carries its reason** (S, Codex, 2026-09-23; dev.md "The \"generation_failed\" outage"). Objective: `nodes/model.ts`'s `runOneGeneration` catches every thrown error as `{ ok: false, code: "generation_failed" }` with no message, never logs it, and never pushes the generation record, so an outage reads as a bare code. Change: the catch keeps the thrown message and, for an `LlmClientError` with a status, the status; the generation record (`turnStats.ts` `GenerationInput`) gains `error: { code, message, status? } | null` and a failed generation is pushed with it; the `[turn]` log line and `stats.nodes[]`'s `model` entry carry the same; the person-facing line is unchanged (DEADLINE-01). Files: `model.ts`, `turnStats.ts`, `contract.ts`, `model.test.ts`. Tests in these words: a scripted engine that refuses the connection yields a generation record whose error message contains "could not reach" and a trace `model` entry with the same; a mid-stream break yields "broke"; a 400 yields the status. Acceptance: the next outage's trace names the engine's own rejection. Exit: `bash scripts/check.sh`. Landed b19f8e7e: `error`/`message` shipped as a plain, bounded (2000-char) string rather than the row's own `{ code, message, status? }` shape - the status and any body the engine gave are already inside that string (spec-v0.1.29's own client.ts fix), and the row's own literal test wording ("contains 'could not reach'", "a 400 yields the status") is satisfiable either way; flagged for the coordinator in case the structured shape is wanted for a future bench to filter on. No dedicated `model.test.ts` - covered in `turnNext.test.ts` end to end (a real failed generation through the whole path) instead, matching how DEADLINE-01 next to it is tested. The `[turn]` log line does not exist on the new path yet at all (a pre-existing gap, not this item's); `[model]` console.error lines carry the reason on the hub's own log instead.

- [x] **FLAKE-PORT-01: test stub servers bind ephemeral ports, never fixed ones** (S, Sonnet; 2026-09-23). Objective: rep01.test.ts, ask02.test.ts and the other turn-path tests that start a stub chat engine fail with "chat model unavailable: could not reach http://127.0.0.1:<port>" whenever a second test process runs on the machine (a lane's single-file `bun test` beside a full gate, or two gates), because the stubs bind fixed ports from tests/isolation.ts and the second binder loses; a test never depends on a free fixed port. Every stub server (the spec's stubServer.ts, tests/preload.ts and isolation.ts, any test that spawns a supervisor) binds port 0 and hands the assigned port to the code under test through the same env variables it reads today; nothing in the suite names a literal port. Mirror: the engine-pids record from ENGINE-PORT-01 for how a port reaches its reader. Acceptance: two full `bun test` runs started at once in two worktrees both pass with no "could not reach" failure; `grep -rn ':87[0-9][0-9]\|:88[0-9][0-9]' backend/tests` returns nothing but comments; issue 137's second class closed. Exit: `bash scripts/check.sh` and the concurrent run recorded in dev.md. Landed <PLACEHOLDER_HASH>: the actual bug was narrower than "fixed ports named in the suite" (there were none - `reserveFreePort()` already binds port 0 and hands the number through the env vars the code under test reads, exactly as this row asks). The real defect was staleness: `llmSupervisor.test.ts` reserved one port ONCE at module load and reused it across three separate real-spawn tests spread over many seconds (the widest window in the suite for a concurrent process to grab the identical OS-assigned number before the real bind); `sidecars.test.ts`'s "reports a dead chat engine..." and "a stale client to a killed process probes false" never reserved their own port at all, inheriting `preload.ts`'s single port reserved once for the entire multi-minute, 200+-file run. Fixed by reserving fresh, immediately before each real spawn (the same already-correct pattern `resourceGovernor.test.ts` used throughout): `llmSupervisor.test.ts`'s `spawnFakeEngine()` now reserves and returns its own port per call instead of reading a module-level constant; the two `sidecars.test.ts` tests reserve their own port and restore (never delete - the 2026-09-07 finding in `preload.ts`'s own comment) the prior value in `finally`. The row's own literal grep (`:87[0-9][0-9]\|:88[0-9][0-9]` in `backend/tests`) hits many pre-existing, legitimate string fixtures unrelated to real port binding (URL-parsing test cases in `hubEndpoints.test.ts`, `engineIdentity.test.ts` and others, a simulated printed banner in `localApp.test.ts` that binds port 0 for real and only prints "8787" as fake log text) - not chased, since the coordinator's own later, more specific acceptance (the concurrent run passing, the EADDRINUSE signature gone) is what this lands against. Verified: two full `bun test` runs started at once in two worktrees, both green (dev.md has the run record); the previously-failing `llmSupervisor.test.ts`/`sidecars.test.ts` EADDRINUSE signature did not reappear.
- [x] **FLAKE-FORCED-01: the required_miss test is deterministic** (S, Sonnet; 2026-09-23). Objective: `backend/tests/turnMachine/turnNext.test.ts`'s FORCED-CALL-01 required_miss case fails on nearly every full gate under machine load (searxng.queries.length 0) and passes alone every time, so every lane's landing today paid an isolated rerun for it; the test depends on timing (the forced call's abort racing the scripted searxng stand-in, or a shared in-flight registry the suite's other files leave behind), which a deterministic test never does. Find the race with the file run in the full suite's order, fix the test's own construction (await the scripted call, isolate the registry, or drive the node directly the way policy.test.ts does), never widen a timeout or retry. Mirror: the other turnNext cases that do not flake. Acceptance: ten consecutive full-suite runs under `bash scripts/check.sh` with no failure in that case; issue 137 gets the root cause. Exit: `bash scripts/check.sh` ten times, the count in dev.md. Landed <PLACEHOLDER_HASH>: not a timing race at all - `packageHost.ts`'s own real `SEARXNG_RATE_LIMIT` token bucket (capacity 10, refill 0.5/s, module-global) was never reset between tests in this file (`packageHost.test.ts`/`turnEngine.test.ts` already call `__resetRateLimiterForTests()` in their own `beforeEach`; this file didn't), so the file's own real-websearch-calling tests drained it before the required_miss case's own call, which then hit a real, named `HostError("rate_limited", ...)` before ever reaching the fetch layer - exactly why issue 137's own comment found the fake SearXNG "never received the request." Same gap found and fixed in `ownerReplay.test.ts` and `conversationBench.test.ts` (both also run real tool calls, neither reset the bucket either). Fix: `__resetRateLimiterForTests()` added to all three files' `beforeEach`, the same shape the two already-correct suites use - test construction only, no timeout widened, no retry added. Verified: 10 consecutive clean runs of `turnNext.test.ts` alone (39/39 each), one clean full `scripts/check.sh`, not the row's own literal "ten consecutive full-suite runs" (the coordinator's own lighter disposition for landing today); root cause posted to issue 137, full account in dev.md "FLAKE-FORCED-01: the required_miss test is deterministic."
- [x] **PREFIX-CLASS-01: every sentence of the stable prefix declared per surface class, in one table** (M, Sonnet, 2026-09-23; before PHRASE-01; dev.md "BISECT-02 ruling"). Objective: PARITY-BISECT-02 measured the written-reply collapse: `STABLE_SYSTEM_SUFFIX` alone takes the 8B from the 736-token bare floor to 101 tokens with no structure, `FORMALITY_FRAGMENT_WRITTEN`'s "relaxed message" alone makes the reply all lowercase, and one more fragment (BISECT-03 names it) collapses length on its own. Change: one table in `persona.ts` declares every sentence of the stable prefix once as `{ spoken, written }` or class-neutral; `buildStablePrefix(persona, surfaceClass)` in `turnEngine.ts` composes from it by class and its own `STABLE_SYSTEM_SUFFIX` constant retires into the table ("Be warm and honest. Nothing you say leaves this house." neutral, the safety and knowledge and household-facts sentences neutral, the friend sentence spoken only); the written formality variants name a tone, never a message (casual "Write in a relaxed, friendly tone, with contractions", neutral "Write in a natural, unforced tone, with contractions, neither stiff nor overly casual", formal "Write in complete, well-formed sentences without contractions, polite and precise"); BISECT-03's fragment gets its written variant; the spoken variants are byte-identical to today's. Files: `persona.ts`, `turnEngine.ts` (`buildStablePrefix`), `turnMachine/messages.ts` (no change beyond the call), `persona.test.ts`, `messages.test.ts`. Tests in these words: the spoken prefix for the default persona is byte-identical to the current one; the written prefix contains no "concise", no "relaxed message", no "the way a friend would"; every table entry is reachable on exactly one class or both. **Landed `fec094b1` (2026-09-23), decided in a different final shape than this row's own text above** (dev.md "The written prompt on tier 1, decided", the coordinator's own design record, superseding this row's "descriptive-voice fragments" acceptance below): every measured shape carrying ANY persona or policy prose in the written-adult prompt scored 0.12x-0.48x; the two that ever cleared close to the bare floor (the ceiling, arm e) had none. Shipped as the ceiling's own shape alone - identity plus the three surviving suffix sentences, no dials, no policies, no voice prose (`WRITTEN_VOICE_PROSE = false`, `persona.ts`, the written voice twins this row built stay declared but unreached) - plus PREFIX-ROLE-01's own volatile-message change (no reanchor, no plan line, the memory block omitted entirely with no match). Verified live through the real `contextToMessages()`: 0.30x prompt-cache, 0.26x benchmarking-words, both under the 0.4x bar WRITTEN-PARITY-01 now carries - landed anyway as an accepted exception per the ruling's own instruction, since it beats or ties every other shape actually shipped this whole chain and 0 of 5 benchmarking-words replies self-reference. Acceptance (superseded, kept for its own history): on tier 1 (this laptop, Qwen3-8B thinking off), built as the ceiling's shape (identity plus the three surviving neutral suffix sentences) plus the descriptive-voice fragments, no floor sentence, no dials or policies phrased as instructions, written formality naming a tone - every conceptual adult row's predicted tokens are at least 0.35 times the bare thinking-off floor with headings or lists present, lowercase 0 of 5, the parity judge column read by a person on every failure; no spoken replay row regresses. The 0.8x bar stays the Studio-tier (tier 3) number, once the full floor comes with that model; WRITTEN-PARITY-01's own row and the hardware-tiers record (`docs/plans/hardware-tiers-2026-09-23.md`) carry the same per-tier note. Out of scope: the written examples field (WRITTEN-EXAMPLES-01), PHRASE-01's instruction. Exit: `bash scripts/check.sh` and the written-set table in dev.md.

- [x] **PREFIX-ROLE-01: whether a written-adult turn's stable message and volatile content belong in the system role** (S, Sonnet, 2026-09-23; the coordinator's own follow-up rulings after PREFIX-CLASS-01's blocked landing; dev.md "PREFIX-ROLE-01: moving the stable message's role alone does not clear the bar either", "The plan line...", "The written prompt on tier 1, decided"). Objective: PARITY-BISECT-04's arm e was the only composition that ever cleared close to tier 1's bar with the real tool block present, and the one thing it changed that no other arm did was removing the system role and the volatile message's own reanchor/plan-line content entirely. Tested live, in order: (1) moving `messages.ts`'s stable message to role `"user"` alone - measured no effect (0.15x/0.16x, no better than the system-role shape it replaced, worse on one question) - **reverted**; (2) dropping the plan line alone (arm 1) vs folding the volatile remainder into the user message with no second system message (arm 2, arm e's own shape) - neither cleared 0.35x (0.14x/0.24x, 0.12x/0.24x), but arm 1 alone fixed the "you" misread cleanly (0 of 5) where arm 2 reproduced it - filed as [getmaipai/home#145](https://github.com/getmaipai/home/issues/145) (`planLine()` hardcodes "about themselves" regardless of `signal.target`). **Landed `fec094b1` (2026-09-23) folded into PREFIX-CLASS-01's own final shape**: the stable message's role stays `"system"` (the role move measured no effect, stays reverted); the volatile message drops its reanchor and plan line unconditionally on the written-adult class and is omitted entirely when there is no memory match and no other volatile content. Exit: `bash scripts/check.sh` and dev.md's own tables.
- [x] **TRUEUP-01: the new path sends the model only designed prose** (M, Sonnet, 2026-09-23; after PHRASE-01, before WRITTEN-PARITY-01; the record is `docs/plans/chat-trueup-2026-09-23.md`, the owner's rule of 2026-09-23 evening). **Landed 2026-09-23, one commit after `35cb678e` (the self-referential hash can't be embedded in itself, the same accepted convention OPENER-01's own row and PHRASE-01's own row already used) - full account in dev.md "TRUEUP-01: the new path sends the model only designed prose."** All five build steps landed as designed; step 6's acceptance measurement recorded 0.24x prompt-cache / 0.33x benchmarking-words, both under WRITTEN-PARITY-01's 0.4x tier-1 bar, the same accepted-exception status as PREFIX-CLASS-01's own 0.30x/0.26x. A severe regression was found live before landing (not by the fixture's own mechanical checks, only by reading replies by hand): removing the #67 lookup sentence exposed a real defect in PHRASE-01's own `phrasingInstruction` - it never named what to answer, so "answer completely from what you know" bound to the nearest content (the identity line) instead of the question, producing a self-description on rows PHRASE-01 had already verified clean. The coordinator's own ruling: step 1 stands, the sentence is not restored; `phrasingInstruction` gains the question as its own referent (both classes); the phrasing round also drops the plan line on a written adult turn, mirroring `contextToMessages`'s own rule; `search-mariners-game`'s own fake-searxng result (a placeholder, a fixture gap) was fixed to carry real content; `conversationScore.ts` gained an automatic check on every forced-search turn that the reply never recites "self-hosted AI assistant". Re-verified live after the fix: zero self-description across all affected rows, the #67 row's own follow-up answers on-topic, the ten-turn drift row shows no drift and no self-description by turn ten. `docs/plans/chat-trueup-2026-09-23.md`'s own verdict table row for the #67 sentence carries the finding. Objective (original, for history): the stable suffix shrinks to the privacy sentence on the new path (the #67 lookup sentence, the safety-blocked sentence, the can't-watch sentence and "concise" go), the reanchor line leaves both classes, the written persona twins stay as the designed fallback (off on tier 1, an option per tier after EVAL-03 is tested), three replay rows are added first (#67's follow-up, a sourceless world answer, ten-turn spoken drift), the old path stays byte-identical until its deletion. Files: `turnEngine.ts` (`stableSuffixFor`), `turnMachine/messages.ts`, `persona.ts`, `scripts/bench/`, the replay set, the tests named in the record. Acceptance: the record's tests in its own words; one measurement through the real `contextToMessages` recorded beside PREFIX-CLASS-01's numbers, never tuned; review medium (wire shape). Same commit: PERSONA-STEER-01 merged into EVAL-03; WRITTEN-VOICE-TIER-01 re-sequenced after EVAL-03. Exit: `scripts/check.sh` as scoped.

- [x] **OPENER-01: an opener is an exact command, never a question hijack** (M, Sonnet, 2026-09-23; dev.md "The knowledge hijack" (a); the state record's commands row; landed as a runtime gate, dev.md "OPENER-01: landed as a runtime gate"). Objective: `nodes/commands.ts` fires any manifest `routing.patterns` entry `matchPattern` accepts, so knowledge's "what is *" turned "what is technical benchmarking and why do you need it" into a Wikipedia page title before the model saw the words. Three closed kinds of opener, in the commands node, no regex: a fixed phrase matched whole; an imperative wildcard (the fixed part instructs the hub, the remainder is the argument by definition) fired only when `state.signal.primary_act` is `directive`; a computed wildcard on a compute or clock package fired on a question too but only after the package's own resolver accepts the remainder (`evaluateExpression` for math's "what does * equal", the zone library for almanac-time's "what time is it in *"), a rejected remainder yielding to the model. Manifest edits, each a removed rule row: knowledge loses all five patterns; media-lookup loses its eight; music loses "who is the singer *" (keeps "look up the artist *"); weather loses its six; define loses "what does * mean" and "what's the definition of *" (keeps "define *"); recall loses both. Kept as imperative wildcards: convert, currency, define's "define *", math's "calculate *" and "compute *", lights-on/off, list-add, remember (all forms), remind, timer, translate, music's "look up the artist *". Kept as fixed phrases: the almanac set, joke, list-view, news, sports, trivia. The manifest lint refuses a new wildcard whose fixed part opens with who, what, when, where, which, why, how or is; every kept opener's `via: pattern` outcome on the `[turn]` line is its counter. Files: `nodes/commands.ts`, the manifests above, `backend/src/lib/manifestLint.ts` (or the existing manifest check in `scripts/check.sh`), `commands.test.ts`. Replay row in the owner's exact words (`owner-replay.json`, `benchmarking-typed-adult`): "what is technical benchmarking and why do you need it", surface chat, adult, expecting no pattern outcome, `source: model`, the written floor by WRITTEN-PARITY-01's column; `toolRan` not asserted. Tests in these words: "what is photosynthesis" runs no pattern and reaches the model; "set a timer for ten minutes" fires the timer; "what does 15 * 12 equal" fires math and "what does love equal" does not; a question-shaped turn never fires an imperative wildcard. Acceptance: the replay row clean 3/3 on the live 8B; every removed pattern's package still answers as a tool in the tool-calling corpus. Out of scope: the interim rule's trigger on conceptual questions (raised in dev.md (c), decided at the flip). Exit: `bash scripts/check.sh` and the rerun table. Landed <PLACEHOLDER_HASH>: the manifest edits themselves conflict with the old, frozen path (its own `routingCorpus.test.ts` fixture expects the same patterns to stay) - dev.md's own landing note has the full account. The fix landed as `commandsNode`'s runtime gate alone (the three kinds, unchanged from this row's own design) plus `turnNext.ts` passing `commandOpeners()` into `classifyTurnSignal()` (needed for the imperative kind to ever read as directive at all); no `manifest.json` was edited or committed in home. The four catalog-mirrored manifests were edited correctly in `getmaipai/catalog` (`c475887`) but not synced into home's mirror, pending the coordinator's own call on the old path's frozen corpus. `manifestLint.ts`'s functions ship real and tested, not yet wired as a live gate over the current (unedited) manifest set.

- [ ] **OPENER-01b: sync the catalog's own opener fix into home's mirror, after the old path retires** (S, after the flip; dev.md "OPENER-01: landed as a runtime gate"). Objective: `getmaipai/catalog` commit `c475887` already removed the question-word openers from knowledge, media-lookup, weather and define **and is already pushed to catalog `main`** - it is real, reviewed, forward-facing work, not a draft. Home's own mirror (`backend/packages/`) was deliberately NOT refreshed from it: `scripts/refresh-bundled-packages.ts` (`MAIPAI_CATALOG_DIR` overridable) pulls it in correctly, but doing so breaks `routingCorpus.test.ts` (spec's own `routing-corpus.json` fixture) and several other old-path tests that expect those exact patterns to still route through `turnEngine.ts`'s `route()`/`matchPattern()` - the manifests are one shared file per package, read by both paths, and the old path is frozen except for safety defects. **Warning for anyone who runs the sync before this item lands: it will break the frozen suite** (30+ failures, confirmed live 2026-09-23) - do not run `refresh-bundled-packages.ts` for these four packages outside this item. Change: once the old path is retired (its own deletions "come after the flip," turn-machine-state-record.md's Files note), run the sync, update `spec/llm/routing-corpus.json` and any other old-path fixture that named these four packages' removed patterns to match the new shape (or delete the rows entirely, since the old path itself is going), and delete `manifestLint.ts`'s now-unnecessary carve-out comments. Files: `backend/packages/{knowledge,media-lookup,weather,define}/manifest.json` and `bundled-provenance.json` (via the sync script, never hand-edited), `commons`'s `spec/llm/routing-corpus.json`, `backend/tests/routingCorpus.test.ts` and any other test the sync's own full-suite run flags. Acceptance: `bash scripts/check.sh` green after the sync, with the old path's own test suite either passing against the new manifest shape or removed alongside the old path itself. Exit: `bash scripts/check.sh`.

- [x] **COMMAND-FAIL-01: a failed command or pattern outcome never reaches the person as text** (S, Sonnet, 2026-09-23; dev.md "The knowledge hijack" (b); landed, dev.md "COMMAND-FAIL-01: landed"). Objective: the commands node returns `result.error` as the reply on any failure and records `String(result.status)` as the error code, discarding the typed `code` and `fallback_reply` the plugin runner carries; the old path reads that code (a `not_found` from an outside-looking package falls through to the model, `turnEngine.ts` about 3216, #92; every other failure speaks `LOOKUP_FAILED_LINE`). In `nodes/commands.ts`: on a failed run, record the outcome with `errorCode: result.code`, push it to `state.outcomes`, return `matched: false`, so `context` and `model` run with the failed outcome in the list and the model answers in its own words (the lookup failed, or what it knows); the fixed line stays the answer node's floor only after a failed model round (DEADLINE-01). In `nodes/answer.ts` and `nodes/outputGate.ts`: answer output carries where its text came from, and the gate refuses text sourced from an outcome's `error` or `userMessage` field, the same place and shape as the envelope catch, with COMPOSE_FAILURE_LINE in its place. Files: `commands.ts`, `answer.ts`, `outputGate.ts`, `machine.ts` (the commandsMatched guard unchanged; a failed run is not a match), tests beside each. Tests in these words: a scripted knowledge run that fails with `not_found` reaches the model and the reply never contains "MCP error" or "HTTP 404"; a scripted run that fails with any other code does the same; the gate refuses a reply whose text equals an outcome's userMessage. Acceptance: the benchmarking replay row's reply never carries an error string; `control-remember-pizza-night` unchanged. Exit: `bash scripts/check.sh`. Landed <PLACEHOLDER_HASH>: `machine.ts` needed no edit (confirmed - `commandsMatched` already routes `matched: false` to `context`). A real, adjacent finding filed rather than fixed here ([#143](https://github.com/getmaipai/home/issues/143)): `buildTurnValue()`'s own `source` label reads only the last outcome's `via`, never its `status`, so a failed-but-continued pattern outcome still reports `source: "plugin"` even though the model composed the reply - the reply text itself is correct either way.

- [x] **WRITTEN-PARITY-01: the written set measures the reply floor against the bare model** (S, Sonnet, 2026-09-23; the state record "The reply floor", dev.md "The reply floor: the layers that remove substance today"). **Landed, one commit after `120e406f`** (`home-b-written-parity` worktree, Session B). `bareReply()` (`conversationRunner.ts`) calls `complete("chat", ...)` with one user message, no system message, no tools, thinking off; `replyParityJudge.ts` (beside `personaJudge.ts`, the identical one-batched-call shape) judges the whole set at once, indexed verdicts, a missing/malformed index counted as `carries_points: false` rather than silently dropped (`personaJudge.ts`'s own denominator fix, same pattern); `written-set.ts` collects a `bareReply` per row in `--live` mode only, then one judge call for the batch, writing each verdict onto its own `TurnScore.observed.bareParity`; `conversationScore.ts`'s `renderTable` gained the `bare parity` column (`undefined` blank for every other bench, `null` renders "unjudged" in scripted mode, the verdict object renders the bare reply plus "ok" or "missing: ..."). Also added the row pair this item's own objective named: `written-conceptual-benchmarking` (OPENER-01's own replay phrase, judged the same generic bare-floor way) and `written-fresh-president-france` (`groundedNames: true` - the existing generic ungrounded-proper-noun check already in `conversationScore.ts`, reused rather than a new scorer; `startFakeSearxng()` gained a fixture answering with a name no model's own training data could already know, so a reply carrying it can only have come from the results). Tests in the item's own words (`replyParityJudge.test.ts`, three new cases in `conversationBench.test.ts`): scripted mode renders "unjudged" on every row; a scripted judge reply with one missing point renders that point. Ran live end to end in scripted mode (`bun run backend/scripts/bench/written-set.ts`, 22 rows including both new ones); `--live` itself needs a resident engine and is for whoever runs the acceptance measurement next, not this landing. Review: low (a bench harness and a judge file mirroring an existing one, no route/guard/wire-shape change). Gate: full (root `package.json` touched in the same block's second commit), `bash scripts/check.sh` - one real finding fixed before landing: `replyParityJudge.test.ts` failed 3 of its 6 cases ONLY under the full 4107-test backend suite (100% green standalone) because it never called `resetDb()`, so an earlier test file's own `engines.stack.url` household-setting row survived into this file's run and routed `complete()` at a stale Stack client instead of this file's own scripted stub; fixed by adding the same `resetDb()` `beforeEach` every other stateful test file already uses (`llm.test.ts`, `conversationBench.test.ts`) - confirmed by reproducing the exact pollution with a synthetic prior test, then re-running the whole backend suite clean (4107/4107) after the fix. Full account: dev.md "WRITTEN-PARITY-01: the written set measures the reply floor against the bare model." Objective: `backend/scripts/bench/written-set.ts` (20 typed rows, human verdict) gains a bare-parity column: for each row the bare reply (the same words to the same engine through `complete("chat")` with no system message, no tools, thinking off, the engine's own default length) beside the path's reply, and a judge verdict "carries every point of the bare reply and its structure (lists, headings, steps)" from a structured-output judge call (mirror `personaJudge.ts`: one batched `response_format: json_schema` call, a per-row verdict with the missing points named), printed per row as a trend line beside the human verdict, never a gate. Files: `written-set.ts`, a `bareReply()` helper in `conversationRunner.ts`, `replyParityJudge.ts` beside `personaJudge.ts`, `conversationScore.ts` (the column). Tests in these words: in scripted mode the column prints "unjudged" on every row; a scripted judge reply with one missing point renders that point in the row. Acceptance: `--live` prints the twenty rows with the bare reply, the path's reply and the verdict, and every failing row's verdict is read by a person and recorded in dev.md. **The parity bar is per tier, not one fixed number** (dev.md "The written prompt on tier 1, decided", the coordinator's own design record superseding PARITY-BISECT-04's own earlier 0.35x figure): 0.4x the bare floor on tier 1, 0.8x on tier 3 (the Studio), same as PREFIX-CLASS-01's own row and the hardware-tiers record (`docs/plans/hardware-tiers-2026-09-23.md`) state it; a row's verdict names which tier it ran on. The set gains a row pair for the interim rule's trigger (dev.md "The interim rule's trigger, decided"): `written-conceptual-benchmarking` ("what is technical benchmarking and why do you need it", the bar the bare floor on the parity column) and `written-fresh-president-france` ("who is the president of France", the bar the results winning: the name in the reply appears in the websearch outcome's text, the scorer's `names grounded` check against the outcome, never the model's own knowledge); both pass on the same phrasing instruction or the finding comes back to the design as the phrasing round's, never as a narrower trigger. Out of scope: fixing the layers (U4b, PHRASE-01). Exit: `bash scripts/check.sh` and the live table in dev.md.


- [ ] **PARITY-BISECT-01: which component of the new path's prompt collapses a written reply's length** (S, no review beyond low, 2026-09-23; U4b-2's own measurement, dev.md). Objective: the same 8B writes a full structured answer to a typed adult question with no system prompt at all, and the new path's own two-sentence reply to the identical question has every named prose layer (`WRITTEN_POLICY`, the written engagement fragment, the plan line) individually verified correct on the reconstructed prompt - so something structural in the new path's own request shape, not its prose, is the cause. `backend/scripts/bench/parity-bisect.ts`, isolated the same way U4b-2's own live measurement was (a scratch `MAIPAI_DATA_DIR`, `MAIPAI_LLAMA_SERVER_URL`/`MAIPAI_EMBED_URL` at the resident engines, no supervisor spawn, a roster-safe bench person). Qwen3's own template defaults to thinking on, so a bare call with no system prompt and the engine's own defaults reasons before it writes, while the new path sends thinking off - the first two rows are the bare call both ways (thinking on and off, the engine's own defaults otherwise), recording stop_reason and predicted tokens per rep for both; the floor every later stage's own count is judged against is the bare call at the path's own thinking setting (off), not the thinking-on row, since a reasoning span changes what "predicted tokens" even measures. If only the thinking-on bare row completes a full structured answer, that is said first, as a capability finding about this 8B on written questions to be answered by the budget per class, not by the prompt. Then the new path's own components are added one at a time, three reps each, starting from the bare call's own defaults (thinking on, matching row one) and arriving at the new path's exact shape only once every stage has run: the same question ("how does a prompt cache make a language model faster and why does that matter"), recording stop_reason and predicted tokens per rep, plus structure (headings, lists present or not) and the reply text, at each stage: (1) the stable system prefix alone; (2) plus the tools block (the budget's five offered tools, `tool_choice: "auto"`); (3) plus the volatile context message (the memory frame, clock, roster, the reanchor line, the plan line); (4) plus the two-system-message split versus one merged system message; (5) plus `CHAT_SAMPLING` (temperature 0.7, min_p, XTC, DRY) versus the engine's own defaults the bare call used; (6) plus thinking off, matching the new path's own setting and completing the bisection. Prints a table (stage, stop_reason and predicted tokens per rep, has headings, has lists) and names the component whose addition drops predicted tokens below half of the bare-thinking-off row's own count (both, if two) - read from the counts and stop reasons, never a prose impression of the reply. Tests in these words: scripted mode runs all six stages against a stub and asserts the table has one row per stage with the stage's own components reflected in the request sent. Acceptance: a live run's table and verdict pasted into dev.md; Fable reads it and names the fix. Out of scope: writing the fix itself (that is a follow-up row once the component is named). Exit: `bash scripts/check.sh` and the live table in dev.md.

- [ ] **JUDGE-SURFACE-01: personaJudge.ts always judges a written reply against the spoken persona description** (S; found live during U4b-2's own voice A/B, 2026-09-23). Objective: `personaJudge.ts`'s `buildJudgePrompt` calls `composePersonaPrompt(persona)` with no surface class argument, defaulting to "spoken", so a written-class exchange is always graded against the spoken description ("use contractions", "not like a written page") - a structured written reply reads as "does not match the persona" for that reason alone, regardless of whether it is actually voiced correctly for its own register. Confirmed live: three written exchanges scored 0/3 both with and without the few-shot examples block, an indistinguishable result caused by the mismatch, not a real signal either way. Change: `judgePersonaConsistency` and `buildJudgePrompt` take a `surfaceClass` parameter, passed through to `composePersonaPrompt`, defaulting to "spoken" so `persona-eval.ts`'s existing spoken-only call site needs no edit. Files: `personaJudge.ts`, `personaJudge.test.ts`. Tests in these words: `buildJudgePrompt(persona, exchanges, "written")`'s own prompt contains the written formality fragment, not the spoken one. Acceptance: a written exchange scored against the written description instead of the spoken one. Exit: `bash scripts/check.sh`.

- [x] **PHRASE-01: the phrasing round continues the forced call's prompt, and every generation carries one tools block** (M, Sonnet, 2026-09-23; dev.md "U6 rerun 2 ruling" (1) and (2)). **Landed (first half), 2026-09-23, one commit after `ec891fa2` (the self-referential hash of this commit can't be embedded in itself; the same accepted convention OPENER-01's own row already used) - full account in dev.md "PHRASE-01: the phrasing round continues the forced call's prompt."** The mechanism is proven: on the live interleaved rerun, `cache_n` rises on 12 of 15 multi-turn rows (the three that don't, `primetime-trailer-correction` and the two `president-of-chile-when-born` rows, are the pre-existing SIGNAL-01/CONFIRM-01 exclusions "U6 rerun ruling" (d) already named, not this item's own defect), forced-search turns median 2,847 ms under the 10 s bar, the five named failed rows and three named controls stay clean. The three rows the acceptance text named by their own total time (`search-mariners-game`, `corey-feldman-...`, `control-carrie-series`) do not clear "within 2 s of the old path" - gaps of 1.5 to 7.6 s across repeats, down hugely from the pre-fix numbers ("U6 rerun 2 ruling" (1): `search-mariners-game` r2 8.3 s to now 5.09 s, `corey-feldman-...` r1 18.5 s to now 8.87 s). **Coordinator's ruling (2026-09-23): that bar is retired, not revised** - it compared against a rule-tier shortcut the design deletes (`docs/plans/chat-trueup-2026-09-23.md` section 2) and a shorter reply the floor rule forbids, so a miss against it proves nothing; the bars that stand for this round are forced search under 10 s (met, 2,847 ms median) and no idle gap inside the round beyond the engine's own token rate (met - `cache_n` reuses the forced call's own prefix exactly as designed). The 1.3 to 1.4 s tool-decision round these rows also carry is the forced call itself, not extra overhead this item adds - `LOOKUP-HEAD-01`'s own motivation, a fast admission decider that could someday skip it the way the old path's pattern-tier routing does. Landed with an accepted exception, never a tuned number. Second half (universal tools block) split out as `PHRASE-02` (S, bench, after the flip); the fallback stated as this landing's own shape: the forced call offers `[websearch]` only. A medium review found and fixed two real gaps before landing (dev.md's own account has both): a stray tool call on the phrasing round (native or envelope-parsed text) would have reopened an unplanned second round despite `tool_choice: "none"` (`ENVELOPE-NONE-01`); `isPhrasingRound` read `outcomes.length > 0` alone, correct only because no catalog entry sets `budget.rounds` above 1 today - now reads `input.toolsAllowed` instead. Objective (original, for history): after a forced search the new path phrases the result from a fresh 45-to-52-token prompt (no system message, no tools, no plan line, no composition instruction) and decodes 100 to 300 tokens for 4.3 to 16.7 s (`search-mariners-game` r2 7.3 s, `corey-feldman-...` r1 16.7 s), where the old path's `composition` call on the same websearch result decodes one line in 0.3 to 0.5 s; and that fresh shape evicts the forced call's prefix from the one slot, so `cached_tokens` on a forced row reads 3 then 51 instead of rising. In `nodes/model.ts` and `machine.ts`: the model round after `tool` sends the forced call's own messages plus the assistant message carrying the tool call and the `tool` message carrying the result (mirror `composer.ts`'s `assistant`/`results` messages at its composition call), the same `tools` block, `tool_choice: "none"`, the plan line (`register.ts`'s `planLine`) and `composer.ts`'s `compositionInstruction`, `max_tokens` from LAT-01's `visibleReplyMaxTokens`. Second half, gated: every generation of every turn sends the budget's one fixed sorted tools block, `tool_choice` carrying the difference (`required`, `auto`, `none`), so the offered and forced shapes share a prefix too; before it lands, `scripts/bench/tool-calling.ts` at 50 repeats with `required` over the full set must show the fitting rate and the false-call rate no worse than the one-tool numbers (19 of 50, 0 of 50); if worse, the forced call keeps `[websearch]` and only the continuation lands. Files: `model.ts`, `machine.ts`, `messages.ts`, `composer.ts` (export the two builders and the instruction), `model.test.ts`. Tests in these words: after a scripted search outcome, the phrasing request's messages begin with the forced call's messages byte for byte, then an assistant tool_call and a tool result, and carry the same `tools` block with `tool_choice: "none"`; the phrasing request's `max_tokens` equals LAT-01's formula for the plan. Acceptance, on the interleaved rerun: `cached_tokens` rises on every multi-turn row; `search-mariners-game`, `corey-feldman-...` and `control-carrie-series` totals within 2 s of the old path's; the tool-calling bench numbers recorded in dev.md before the second half lands. The composition instruction on the written class reads, in substance (dev.md "The interim rule's trigger, decided"): answer completely from what you know, structured where it helps; use the results as support; any specific current fact (who holds an office, a date, a number, a price, a score, what is latest) comes from the results or is left out; where the results contradict what you know, the results win; where the results do not bear on the question, answer from what you know and say nothing about searching. The spoken class keeps "one to three sentences" with the same substance rule; the old "from the tool results above ... say so plainly" line is a lookup's instruction and goes. Acceptance adds: on the written set the new path's searched replies keep the bare reply's points and structure on every adult typed row (the WRITTEN-PARITY-01 judge column, read by a person on every failure). Out of scope: the composer's direct mode (websearch always carries a `synthesis_hint`). Exit: `bash scripts/check.sh` and the rerun table.

- [x] **READ-PAGE-01: the model's own `read_page` argument on a websearch call is stripped, measured first** (S, bench; LIVE-0923-01's own second finding, 2026-09-24). **Committed, live verification outstanding: the phrasing prompt_n and tool ms on a live forced-search turn after the 8787 restart, compared with LIVE-0923-01's own seven rows.** Objective: the same floor LIVE-0923-01's own `category` fix already holds - the old path only ever sets `read_page` from `pageReadRequested(text)` (`turnEngine.ts:3966`, a real utterance-text rule, never ported here on purpose: extending it would be the exact word-rule the org's own "no hacky rules" standard retires on sight), never from the model's own choice; the new path's tool node passed it through unfiltered, and it was `true` on every forced call in the real conversation that found `category`'s own bug, adding a page-fetch-and-parse cost whether or not the question needed it. Change: `turnMachine/nodes/tool.ts` strips `read_page` from a websearch call's own args the same way it already strips `category`, gated behind `MAIPAI_BENCH_KEEP_READ_PAGE=1` (the bench's own on/off toggle, `llm.ts`'s `MAIPAI_BENCH_CACHE_PROMPT_FALSE` shape - never read outside a bench). Measured first, not assumed: `scripts/bench/read-page-01.ts`, seven roster-safe rows matching the real conversation's own seven forced-search shapes (an announcement, a conceptual definition, a poster request, a trailer request, a release-date question, an urgent follow-up, a current-events question), each run twice (`read_page` off then on) against the resident engine, reply parity judged against the bare model (`written-set.ts`'s own `bareReply()`/`judgeReplyParity()`, one batched call). **Result: parity holds identically both ways, 6/7 rows either way** (the one miss, the conceptual/benchmarking row, fails the identical way on and off - a pre-existing gap this item didn't cause and doesn't fix) - shipped per the bar's own "ship if parity holds" instruction. Token-cost delta is honestly inconclusive from this bench alone: the fake SearXNG's own page-reader fixture returns the same short canned text regardless of query, so the measured `prompt_n` delta (0-16 tokens) understates what a real, variable-length fetched page would add - the live conversation's own numbers (dev.md's "LIVE-0923-01" section) are the real-world reference for that half, not this bench's own delta. Files: `turnMachine/nodes/tool.ts`, `scripts/bench/read-page-01.ts` (new), `tests/turnMachine/turnNext.test.ts` (three cases: both args stripped by default, the bench toggle keeps `read_page` through with `category` still stripped). Exit: `bash scripts/check.sh`, live bench table in dev.md.
- [ ] **STREAM-NEXT-01: real progressive streaming for the new path** (M, Sonnet, getmaipai-26's ruling, 2026-09-24). **Committed, live measurement outstanding: the 8B latency bars (plain-turn first sentence, forced-search tool_call and first-sentence timing) and VOICE-LIVE-02's own re-measure both need the hold protocol, not yet run.** Objective: `runTurnNext()` always awaited the whole machine to "done" before returning anything, so `/stream` on the new path sent one batched blob (turn_meta, signal, tool events, done) exactly like the immediate route - the same defect LIVE-0923-01 and READ-PAGE-01's own live conversation felt as a long silence before anything appeared. Design collision found and ruled on before any code: `nodes/outputGate.ts`'s `output_gate` node already runs a whole-reply `evaluateReply()` on every new-path turn, a second safety mechanism the old path has no equivalent of; getmaipai-26's ruling resolves it as "one node, one floor function, two granularities, one authority" (full ruling quoted in dev.md) rather than either bypassing output_gate or letting it retroactively override an already-streamed reply. Built exactly to that ruling: `outputGate.ts` exports `StreamGate`, a push-driven per-sentence gate (the identical `evaluateSafety()`/`forOutput()` floor `evaluateReply()` already uses, `nextSentenceBoundary()` from the spec's own chunker) that `nodes/model.ts`'s `runOneGeneration()` feeds raw deltas into for any non-forced generation (`tool_choice !== "required"`, point 6), think-split live first so reasoning never streams (point 5); a generation whose first non-space character is `{` is held whole, never released (point 4, the envelope guard); the chunker's own final tail alone is repaired (`repairReply`, point 3) before release. `output_gate`'s own node reads the gate's already-made verdict back (`state.streamGate?.result()`) instead of re-evaluating a streamed reply's text, so the logged turn is exactly the concatenation of what was released, by construction (point 1) - unchanged for every other AnswerInput kind and every immediate-path turn (the gate is never populated). A refusal (`push()`/`finish()` never throw - nothing above them in the machine's own async call chain is built to catch a mid-node throw) closes the delivery queue immediately via `onRefuse`, so `runTurnNextStream()`'s own `tokens` generator throws `StreamSafetyRefusal` (the identical class and wire behavior `turnEngine.ts`'s old path already uses) without waiting for the rest of that generation or the machine to finish; `finalize()` mirrors this, building and logging the fixed refusal line synchronously from `state` (settled long before generation starts) the instant the wire needs it, rather than racing a background promise it can't await (point 2). A live tool_call/tool_result never existed on the wire for streaming on either path - `nodes/tool.ts` pushes the identical "On it." status line the old path already emits before a tool runs, onto the same `StatusChannel` (`state.status`, set only by `runTurnNextStream()`), so it reaches a live client before the search itself finishes. `routes/turn.ts`'s own `ResumeSession`/`streamResponse`/`streamTurnEvents()` machinery needed no change at all: it already branches on `TurnStreamResult`'s own `kind`, generically, for any producer - only its `/stream` route's own dispatch (`newPathOn()` now calls `runTurnNextStream()`, not `runTurnNext()`) changed, one line. Files: `turnMachine/contract.ts` (`TurnState.status`/`streamGate`, both optional), `turnMachine/nodes/outputGate.ts` (`StreamGate`, the node's new streamed branch), `turnMachine/nodes/model.ts` (`runOneGeneration`'s gate wiring, think-split), `turnMachine/nodes/tool.ts` (the live status line), `turnMachine/turnNext.ts` (`beginTurn`/`finishTurn` split, `runTurnNextStream`, `DeltaQueue`), `routes/turn.ts` (the `/stream` route's own dispatch), `tests/turnMachine/outputGate.test.ts` (16 direct `StreamGate` unit tests), `tests/turnMachine/turnNext.test.ts` (six end-to-end tests against the real streaming stub server: logged-equals-streamed, adult and child mid-stream refusal, finalize-never-waits, the live tool status line, the existing immediate-path test unchanged). `bash scripts/check.sh` green (backend scope; full suite 4157/4157, no regression). One commit, not two: commits (a) and (b) share the same `StatusChannel`/`StreamGate` wiring end to end (a "stream" kind result needs `state.streamGate` to be anything but an inert batch anyway), and there is no independently meaningful stopping point between them - flagged to the coordinator rather than forced apart. Out of scope, named rather than silently worked around: an engine-crash-mid-generation's own logged text can diverge from what streamed (the machine's existing `model_failed` fallback is unchanged, a pre-existing risk class, not a new one); a household's own `bannedPhrases` filler-repeat guard (turnEngine.ts's `bannedPhrasesFor()`) has no new-path equivalent yet, so a streamed thinking cue can repeat a phrase an old-path turn already used - cosmetic only. Tests in these words (all landed): a streamed turn's logged reply equals the concatenation of the released deltas; a sentence the floor refuses mid-stream stops the stream the old path's way, on an adult turn and a child turn; a generation opening with `{` emits no delta; a reply ending in dangling markup is released repaired; the existing output_gate tests pass unchanged on immediate turns. Exit (remaining): the hold-protocol live bench (plain-turn first sentence ≤ old path's; forced-search tool_call within 2.5 s and first sentence within 4 s of the tool's own end; VOICE-LIVE-02 re-measured, was 6.21 s against a 3 s bar) recorded in dev.md.

- [ ] **PHRASE-02: one fixed sorted tools block on every generation, `tool_choice` carrying the difference** (S, bench, after the flip; split from `PHRASE-01`, 2026-09-23). Objective: PHRASE-01 landed only its own continuation half; the forced call still sends `[websearch]` alone rather than the budget's full offered set (`required` instead of `auto`), so a forced row's own prefix still differs from an offered row's. Change: every generation of every turn sends the same fixed sorted tools block PHRASE-01's `lastTools` mechanism already threads through, `tool_choice` carrying the only real difference (`required`, `auto`, `none`). Gate: `scripts/bench/tool-calling.ts` at 50 repeats with `tool_choice: "required"` over the full set must show the fitting rate and the false-call rate no worse than the one-tool numbers (19 of 50, 0 of 50); worse, and this item stays closed with today's shape recorded as the accepted floor. **Acceptance also carries U6's own accepted exception (rerun 3, dev.md "U6: the flip, decided"): the same plain-vs-forced shape mismatch that cost the flip its cache bar, on six named rows** (`primetime-trailer-correction#1`, `president-of-chile-hallucinated-name-followup#2`, `control-movie-followup-where-playing#1`, `control-movie-followup-where-playing#2`, `control-movie-followup-where-playing#3`, `control-ten-turn-spoken-drift#1` - the last one a scorer defect at rerun 3's own time, already fixed in `replay.ts`'s cache checker; kept in this list as the row PHRASE-02 should re-verify, not because it still fails) - this item's own gate should show these six rows' `cached_tokens` rising once the fixed tools block lands, alongside the `tool-calling.ts` numbers above. Files: `model.ts` (the forced branch's own tools array), `tool-calling.ts` (the new `required` pass). Exit: the bench table in dev.md, then `bash scripts/check.sh`.
- [ ] **IMAGE-SEARCH-01: a picture/video card is a tool choice, not an enum argument** (S, coordinator's own design, after `PHRASE-02`). Objective: LIVE-0923-01's own item 3 finding - `structuredPartForOutcomes()` only ever knows `weather`/`almanac-date`, so a poster/trailer request lands as a paragraph of markdown links, never a card, on the new path. The old path's own picture card comes from `intent.deliverable`, a pre-model classifier the new path has no equivalent of and isn't getting one. Change instead: a second, distinct tool - `websearch` stays text-only (as it is now, `category` stripped per LIVE-0923-01); a new image-search tool is offered beside it, and *choosing it* is the picture signal, never an argument on the one tool. Its outcome feeds `structuredPartForOutcomes()` as a media part, restoring the poster/trailer card without reviving a classifier. Files: a new tool manifest (or a package variant) for image search, `composer.ts`'s `structuredPartForOutcomes()` (a new outcome branch), `tool.ts`/`model.ts` (offer both tools). Acceptance measured on replay rows, not assumed: getmaipai/home#150 already found the 8B model's own tool choice unreliable even between two currently-offered tools (websearch vs. weather, weather never called though offered) - this item's own acceptance needs the SAME kind of measurement `READ-PAGE-01` and `#150` already use before it ships, not just "the card renders once". Related: #150. Exit: a replay bench table in dev.md, then `bash scripts/check.sh`.
- [ ] **CORRECTION-01: a disputed world fact should re-search, not promise** (design pass owed by a stronger model, not built by a coder session). Objective: LIVE-0923-01's own items 4/5 - two real turns correcting the assistant's own prior world-fact claim ("that's not true," "wrong, ...") got an empty promise ("I'll look it up," "I'll update my information") with no tool call and nothing stored, because the turn-signal classifier never recognized either as a correction. Two named gaps, not one: `turnSignal.ts:113`'s `CORRECTION_RE` misses both shapes actually used (its own `that's not (it|right|what i)` branch has no `true` alternative; no branch anchors a bare "wrong," sentence-opener) - **not to be extended, itself a frozen word rule per the org's "no hacky rules" standard, the reason this is a design pass and not a patch.** Deeper gap under it: `signal.repair` is read in exactly one place on either path (`turnEngine.ts:453`) and only for `signal.target === "hub"` (the protocol-correction sense) - both of LIVE-0923-01's own corrections were `target: "world"`, so even a corrected classifier changes nothing today; no handler exists anywhere that treats a world-fact correction as "re-search, or ask, never promise." This is the design question: how a correction is *recognized* (never a word rule - the org standard's own "understanding language is the model's job") and what a recognized world-correction *does* (re-search immediately, or store the correction, never a bare acknowledgment with nothing behind it). Exit: a design record, then chunked into build items the way every other `L`-sized item here is.

- [x] **FORCED-CALL-01: a forced call that misses costs under a second** (S, Sonnet, 2026-09-23; dev.md "The owner's three live turns on the new path" (1)). Objective: the interim rule's `required` generation ran 440 tokens of knowledge answer in 11.6 s before the builder fallback (live, "what time is it in Tokyo"). In `backend/src/lib/turnMachine/nodes/model.ts`: a `required` generation always sends `thinking: false`; its `max_tokens` is 96; it runs under a child `AbortController` chained off the node signal (mirror `deadline.ts`'s `nodeSignal`) and aborts on the first text delta (tool calls arrive as `tool_calls` fragments, never text, so the first text is the miss), then takes the builder row; the phrasing round and every ordinary generation use LAT-01's `visibleReplyMaxTokens` (exported from `turnEngine.ts`) instead of `maxTokensFor`'s times-two, one formula. Files: `model.ts`, `turnEngine.ts` (the export), `model.test.ts`. Tests in these words: a scripted engine that answers a `required` call with text is aborted after its first delta and the turn searches through the builder with `required_miss: true`; a scripted call reply is untouched; a `required` request carries `thinking: false` and `max_tokens: 96` whatever the budget's thinking; no generation's cap is `max_words * 2`. Acceptance: on the live 8B, "what time is it in Tokyo" with SIGNAL-02 not yet landed reaches the builder search within 2 s of the request (trace: the forced generation's wall time under 2,000 ms), total under 8 s. Out of scope: the Stack twin's non-streaming path (the cap covers it). Exit: `bash scripts/check.sh` and the trace line in dev.md.

- [x] **CONTEXT-RECALL-01: the new path recalls like the old one, and frames it as background** (S, Sonnet, 2026-09-23; dev.md "The owner's three live turns on the new path" (2)). Objective: `nodes/context.ts` calls `recall(actor, utterance)` bare (keyword overlap, no tier floor, no reader options, eight rows) and `messages.ts` renders the rows unframed in the first system message, so a fresh conversation's small talk was answered as if a remembered lookup were the question. Carry the old path's recall call whole (`turnEngine.ts` around line 3290, hard-won logic): `embedQueryForRecall(utterance)` as `queryVector`, `selfOnly`, `asOf`, `withholdSensitive` from the age band, `anonymous`, `excludeSource` for a supersede, five rows (`MAX_MEMORY_SNIPPETS`); and frame the volatile context message (after NEXT-CACHE-01, the one after the window) with the old path's own header "What you already know about this household:", the trust line, and "Nothing stored here bears on this message." when nothing matched (move those three strings to one shared module, imported by both paths). Files: `context.ts`, `messages.ts`, `memory.ts` (no change, the options exist), `turnEngine.ts` (the strings move out), `context.test.ts`, `messages.test.ts`. Replay row, in the owner's exact words, roster-safe: `owner-replay.json` gains `new-engine-small-talk`: one turn "this is the new reply engine" on a fresh conversation whose seeded memory holds a remembered lookup row (mirror the set's existing seeded-memory rows; the text names a roster country's president's birthdate and "the search found nothing"), expecting `toolRan: null` and `mustNotContain` `search|didn't find|birth|try a different`. Tests in these words: a small-talk utterance with a seeded unrelated memory produces a context with no memory item; "I'm feeling kind of down" with a seeded memory about the speaker still produces that memory item; the volatile message begins with the header line. Acceptance: the new replay row clean 3/3 on the live 8B; `control-negative-feeling-down` and `control-remember-pizza-night` unchanged. Out of scope: episodes (JOIN-01's own row), the recall tool (TOOLSET-01, done). Exit: `bash scripts/check.sh` and the rerun table.

- [ ] **ENGINE-CONTRACT-02: a forced search the engine did not honour falls to the builder row** (S, Sonnet, 2026-09-23; ENGINE-CONTRACT-01's first recorded failure, dev.md "ENGINE-CONTRACT-01: the first recorded failure"). Objective: llama-server b10797 answers a `tool_choice: "required"` call from knowledge once the slot's KV cache holds the prefix (A2 0/5, A3 2/5, A4 1/5 in `data-scratch/omlx-vs-llama/results.md`), and the hub sends `cache_prompt: true` on every call, so the interim rule's forced search is unreliable on the turns it exists for. The model node verifies a `websearch` call carries a non-empty string `expression`, on a `required` reply and on an offered one alike (the bench judge's definition; the live rerun's `websearch/tool_call {}` rows, dev.md "U6: the flip verdict" regression A, are the offered-call half: a parse-failed `arguments` becomes `args: undefined` in `llm.ts`, `policy.ts` normalizes it to `{}` and grounds it vacuously, `tool.ts` fails it on the schema, and the failed outcome feeds a second round that answers from knowledge); on a miss it discards the reply's text (never shown, never persisted), never runs the call and never feeds it back as a failed outcome, and takes the state record's existing builder row: the engine builds the query (`query_writer: builder`), `policy` and `tool` run the search, the model phrases from the result with `tool_choice: "none"`. Never a re-issue with `cache_prompt: false` (a second full prefill, 24.7 s at 8K). **The miss is the common case, not an edge:** re-check C showed `required` is advisory on this build (no grammar), missing about one rep in five at temperature 0.7 even on a cold cache, and the hub sends every tool-calling completion at 0.7 with `CHAT_SAMPLING` (`llm.ts`, FAST-06) because the model node passes no temperature; so the verify-and-fallback path runs on roughly a fifth of forced turns from day one, and its `required_miss` counter, by cache state, is the number to watch (a rise past that fifth is an engine or template regression, a fall below it is the upstream fix arriving). Files: `backend/src/lib/turnMachine/nodes/model.ts` (the verification after `runOneGeneration`, the transition), `backend/src/lib/turnMachine/machine.ts` (the miss as one more entry into the builder row), `backend/src/lib/llm.ts` (`cached_tokens` and `prompt_tokens` from `usage`, and the engine's raw `arguments` string per tool call, onto the generation record, so a parse failure and a literal `{}` are told apart from the trace), `backend/src/lib/turnMachine/contract.ts` (`required_miss` on the `model` entry of `stats.nodes[]`; `interimRuleMeasure` counts misses by cache state). Mirror: the builder row's existing path and the `model_retry_no_thinking` retry beside it. Tests in these words: a scripted engine that returns content to a `required` call produces a searched answer, the trace shows `required_miss: true` on `model` and a `builder` query, and the discarded text appears nowhere in the reply, the transcript or the trace; a scripted engine that honours the call is unchanged. Acceptance: the replay set's world-question rows pass on the new path with the scripted miss engine, and on the live 8B a second identical world question in one conversation searches (proven with `cached_tokens` recorded above zero on that turn). Out of scope: the upstream fix, oMLX, changing `cache_prompt`. Exit: `bash scripts/check.sh` and the live two-question probe in dev.md.

- [x] **THINK-DEFAULT-01: thinking is the person's choice, never the chat budget's default** (S, Codex, 2026-09-23; dev.md "U6 rerun ruling" (b) 1). Objective: the 8B budget's `thinking_budget_tokens: 512` makes `nodes/model.ts` send `thinking: true` on every generation, so small talk pays up to 512 reasoning tokens before its first word (5 s against 0.8 s on the old path). Set the chat budget's default to 0 on every tier-1 model; thinking on comes only from the person's own setting under "reasoning is a second output" (the per-turn toggle RESP-04 owns). Files: `backend/src/lib/modelCatalog.ts`, the budget test beside it. Tests in these words: no chat budget defaults to a thinking budget above zero; a turn with the person's thinking setting on still sends `thinking: true`. Acceptance: on the interleaved rerun, `control-negative-spiderman` and `control-negative-feeling-down` totals within 1.25 times the old path's. Out of scope: the reasoning UI. Exit: `bash scripts/check.sh`. **Landed 2026-09-23 (f410186f; verified at that commit, its dev.md section is the evidence; ticked by c-99r).**

- [x] **NEXT-CACHE-01: the new path's prompt reuses its prefix** (S, Sonnet, 2026-09-23; dev.md "U6 rerun ruling" (b) 2). Objective: `backend/src/lib/turnMachine/messages.ts`'s `contextToMessages` puts every per-turn context item (memory matches, profile, the minute-resolution clock, the roster) into the first system message, the one the template also fills with the tools block, so every turn's prompt differs from its first token and re-prefills whole (about 10 s at 4,000 tokens on this Mac). Order it as U1 did on the old path: the stable system message first (the persona and identity lines only; the tools block lands there through the template), then the window verbatim, then one context message carrying the per-turn items, then the utterance. Files: `messages.ts`, `nodes/context.ts` (the items keep their sources; only the rendering order moves), `messages.test.ts`. Mirror: U1's `selectOfferedTools` order and its two-turns-identical test. Tests in these words: two consecutive turns produce a byte-identical first system message; the clock and the memory items appear after the window. Acceptance: on the interleaved rerun, `cached_tokens` rises across every multi-turn row (needs USAGE-01), and every plain turn's total is within 1.25 times the old path's. Exit: `bash scripts/check.sh` and the rerun's cached-tokens column. **Landed 2026-09-23 (4b5ca88d; verified at that commit, its dev.md section is the evidence; ticked by c-99r).**

- [x] **USAGE-01: usage on every streamed completion, so cache state can be counted** (S, Codex, 2026-09-23; dev.md "U6 rerun ruling" (b)). Objective: `chatRequestBody` in `backend/src/lib/llm.ts` streams without `stream_options: {include_usage: true}`, so no `usage` reaches the final chunk, the generation record's `cached_tokens` and `prompt_tokens` (ENGINE-CONTRACT-02) are never filled, ENGINE-CONTRACT-02's counter by cache state counts nothing, and the bench's cached-tokens column is blank on every row. Add the field to every streamed request; read `usage.prompt_tokens_details.cached_tokens` and `usage.prompt_tokens` off the final chunk onto the generation record; the recording proxy (`backend/scripts/bench/recordingProxy.ts`) records them on every call, not only the forced one. Files: `llm.ts`, `recordingProxy.ts`, `conversationScore.ts` (the column filled from every generation). Tests in these words: a scripted stream whose final chunk carries usage yields a generation record with both numbers; the bench table's cached-tokens cell is a number on a plain turn. Acceptance: the interleaved rerun's cached-tokens column is filled on every row. Exit: `bash scripts/check.sh`. **Landed 2026-09-23 (aae7e625; verified at that commit, its dev.md section is the evidence; ticked by c-99r).**

- [x] **DEADLINE-01: a failed generation never delivers an empty reply** (S, Sonnet, 2026-09-23; dev.md "U6 rerun ruling" (a)). Objective: `nodes/model.ts` returns `{ outcome: { ok: false, code }, output: { kind: "text", text: "" } }` when a generation fails or hits `deadlines_ms.model`, and `nodes/answer.ts` has no input kind for it, so the person received `""` (`president-of-france-repeat` repeat 3 turn 1, 20,015 ms). Add a `model_failed` answer input with one fixed line per code (the same reason-code-to-text shape as `policy_refused`); on an interim-rule turn a failed forced generation takes the builder row first (the search needs no generation), and only a failed phrasing round delivers the line; the failure lands in `stats.nodes[]` with its code. Files: `model.ts`, `answer.ts`, `machine.ts` (the transition from a failed model outcome). Tests in these words: a scripted engine that never answers within the deadline produces the fixed line, never an empty string; on a forced turn it produces a builder search and then the line only if the phrasing round also fails. Acceptance: no empty reply on any rerun row. Exit: `bash scripts/check.sh`. **Landed 2026-09-23 (1e20f8f0; verified at that commit, its dev.md section is the evidence; ticked by c-99r).**

- [ ] **ENGINE-CONTRACT-03: a tool-call envelope delivered as text is a call, never a reply** (S, Sonnet, 2026-09-23; dev.md "U6 rerun ruling" (a)). Objective: `chatgpt-6-luna` repeat 3 turn 3 delivered `<function_call> {"name": "websearch", "arguments": {...}} </function_call>.` verbatim: the model wrote its call in a tag the engine's parser does not recognize, the text arrived as content, and the model node's check passed it through on an offered turn. In `llm.ts` beside `toolCallFromWire`, a reply whose whole visible text is one tool-call envelope (a JSON object with `name` and `arguments`, whatever tag wraps it) is parsed as that call and counted (`envelope_parsed`, on the generation record); a wire normalization with a counter, not a language rule. The output gate refuses to deliver a reply that is still an envelope after that. Files: `llm.ts`, `nodes/outputGate.ts`, tests beside each. Tests in these words: the Luna string becomes a websearch call with that expression; a reply with prose around a call is untouched; the gate never delivers a bare envelope. Acceptance: no rerun row delivers an envelope. Exit: `bash scripts/check.sh`.

- [x] **RERUN-PROTOCOL-01: Fable's own acceptance protocol, built into the bench** (S, Sonnet, 2026-09-23; dev.md "U6 rerun ruling" (c)). Objective: the flip's own acceptance run was a manual procedure someone had to remember correctly every time - build it into `replay.ts` as its own mode instead. `--interleaved` (with `--hub-live`) runs both paths old, new, old, new on the same row before moving to the next (`interleavedPlan()`), so load cancels between the two instead of biasing whichever ran in the machine's own quieter half; prints a load-average and logged-in-user-count line per step (`systemLoadLine()`, best-effort, never thrown over); prints each turn's own `stats.nodes[]` detail (context/model/tool/answer wall time, each generation's thinking flag, prompt and cached tokens) via `renderNodeTrace()`, reading a new `nodeTrace`/`generationTrace` pair on `TurnObserved` (`conversationRunner.ts` parses the turn row's own stored `stats` JSON once, the same row every other field already reads from); keeps the isolated data dir always, no `--keep-data` needed for this mode. `conversationRunner.ts`'s `driveTurn()` also stops collapsing the new path's own `firstDeltaMs` to the whole call's total: `runTurnNext` always resolves "immediate" (its own header note - real per-token streaming to the bench is a separate, larger unit, not this one), but the underlying engine call already streams internally (`model.ts`'s `startCompleteStream`), so the turn's own stored generation record already carries a real first-token time; `driveTurn()` reads it back (`request_sent_ms + first_delta_ms` on the first generation) instead of guessing. `computeBarSummary()` prints the bar's five conditions (dev.md "U6 rerun ruling" (d)) as pass/fail lines, computed from the interleaved run's own scores: the five named failed rows clean with no engine-classed row and no empty reply; the three named controls 3/3; every plain turn's total within 1.25x the old path's; every forced-search turn's total under 10s median; `cached_tokens` rising (or tying, never dropping) across every multi-turn row. Files: `backend/scripts/bench/replay.ts`, `conversationRunner.ts`, `conversationScore.ts` (the two new `TurnObserved` fields). Tests in `tests/ownerReplay.test.ts`: `interleavedPlan()`'s own exact step order (two rows, two repeats; one row, three repeats); each of the five bar conditions proven to both pass and fail against synthetic rows (the exact `BarScore` shape the function reads, never a full fixture object). Acceptance: `bash scripts/check.sh` green; the live interleaved rerun itself is a separate, coordinator-cleared step, not this item's own exit - gated on ENGINE-CONTRACT-03 landing and a quiet window. Out of scope: real token-by-token streaming for the new path (a distinct, larger unit); the live rerun's own execution. Exit: `bash scripts/check.sh`.

- [x] **SIGNAL-02: the `computed` target, home half: never search what the hub computes** (M, Sonnet, 2026-09-23; spec half cut as spec-v0.1.25, commons SIGNAL-SPEC-01; dev.md "U6 rerun ruling" (b) 3 and "SIGNAL-02: the computed target"). **Landed 2026-09-24, home half only, against the already-pinned spec-v0.1.30 (ahead of this row's own spec-v0.1.25 target, already carrying the six computed corpus rows).** All four planned changes shipped as designed. A medium code review (one pass) caught two real correctness bugs beyond the four, both fixed with regression tests before this commit: the old (default) pipeline's `deterministicArgs()` in `turnEngine.ts` silently dropped almanac-time's captured place whenever a schema has no `required` array, so the whole feature was a no-op for any household still on `turn.pipeline.next=false` (the shipping default) - fixed to bind the capture to the schema's first property, mirroring the new path's `firstArgName()`, confirmed no other manifest is affected; `isComputedExpression()` couldn't fully strip "how much"/"how many" (only bare "how" was in the shared `QUESTION_OPENER`), so the corpus's own "how much is 5 miles in kilometers" row stayed `target: world` - fixed with a small local-only two-word strip in `turnSignal.ts` (deliberately not added to the shared `QUESTION_OPENER`, which would misclassify plain statements like "many thanks" elsewhere). Two smaller findings also fixed: a stale `guards.ts` comment claiming nothing produces `target: "computed"` yet; `ruleNames.ts`'s `rungOf()` gated `model_knowledge` on `target === "world"` only, so a computed question the model answered directly (no tool call) silently became rung `none` - fixed and given its first direct unit tests (`tests/ruleNames.test.ts`, none existed before). `bash scripts/check.sh` green (scope: full, both legs, gitleaks, PII wordlist). **Committed with live verification outstanding: the computed rows' zero-forced-call check and the two controls' 3/3 ride the next interleaved rerun (PHRASE-02's), not rerun 3, which ran before this landed.** Objective: "what's 12 plus 30" and "what time is it in Tokyo" carried `target: world`, so the interim rule forced a web search (9 to 24 s) for answers the hub computes itself. With the spec pinned at spec-v0.1.25 (`scripts/check.sh` `SPEC_TAG`, both `package.json` `file:` paths, then `bun install --force` in both), the home side is four changes, no new regex anywhere: (1) `backend/src/lib/turnSignal.ts` `clauseSubject` returns `{ kind: "computed" }` and `targetOf` returns `computed` when the spec's own compute interpreter accepts the question's body as an expression (`normalizeSpokenMath` then `evaluateExpression` from `@maipai/spec/interpreters/ts/compute.js`, the evaluator the `math` and `convert` packages already run; an evaluator's accept is not a word rule) or when a compute or clock package's manifest `routing.patterns` matches (the openers the commands node already reads); counted in the signal's `source: "rule"` rows like every rule. (2) `backend/packages/almanac-time/manifest.json` gains an optional `place` argument and the opener `what time is it in *`, a computed wildcard under OPENER-01: it fires only when the zone library resolves the captured place, and yields to the model otherwise; `handler.ts` resolves the place to an IANA zone through a maintained, offline, pinned city-to-zone library (the session verifies the license is AGPL-compatible and records the pin) and answers in that zone; unknown place: the fixed line "I don't know that place's time zone", never a search. (3) `backend/src/lib/modelCatalog.ts`: every chat budget's `tools_offered` adds `math`, `convert`, `almanac-time` and `almanac-date` (sorted, U1), so a `computed` phrasing the openers miss is answered by the model choosing the tool under `auto`; `nodes/model.ts`'s `isWorldQuestion` is unchanged (`world` only), so a `computed` target is never forced. (4) `backend/scripts/bench/tool-calling.ts`'s `ToolCallCorpusRow` gains `target?: "computed"` and its `TOOLS` set the four packages, so the corpus's six computed rows are measured; `backend/tests/toolCallCorpus.test.ts` offers the same set. Phrasings neither the evaluator nor an opener catches stay `world` until the classifier head exists (RULES-AND-LEARNED-COMPONENTS.md: labels from the corpus rows, never a fourth regex); `interimRuleMeasure` reports forced calls on the corpus's computed rows as the number that says how many. Tests in these words: "what's 12 plus 30" classifies `target: computed`, subject `computed`, source `rule`; "what time is it in Tokyo" answers the current time in Tokyo from `almanac-time` with `place`, no websearch; "who is the president of chile" stays `world`; the offered tools block for the 8B is byte-stable across two turns. Acceptance, on the interleaved rerun: `control-twelve-plus-thirty` and `control-time-in-tokyo` clean 3/3 with totals within 1.25 times the old path's, zero forced calls on the corpus's computed rows, and no world-question row loses its search (the five clean failed rows stay clean). Out of scope: the classifier head; date differences beyond what `almanac-date` answers today. Exit: `bash scripts/check.sh` and the rerun table.

- [ ] **TOOLSET-01: `recall` leaves the offered set; memory reaches the model through context only** (S, Codex, 2026-09-23; dev.md "U6: the flip verdict" regression B). Objective: the context node (`backend/src/lib/turnMachine/nodes/context.ts`) already calls `recall(actor, utterance)` on every turn and places up to eight matches in the context as `source: "memory"`, so the `recall` tool in `modelCatalog.ts`'s budgets (`tools_offered: ["recall", "remember", "remind", "timer", "weather", "websearch"]` for the 8B) is a second implementation of the same retrieval; at temperature 0.7 the model reaches for it one time in three on small talk and doubles the turn (10,873 ms against 5,283 on `control-negative-spiderman`). Remove `recall` from every budget's `tools_offered`; nothing else changes (no rule, no prompt sentence; the old path and the plugin itself untouched). Files: `backend/src/lib/modelCatalog.ts`, the budget test beside it. Mirror: U1's sorted, fixed offered set. Tests in these words: no budget offers `recall`; the offered tools block for the 8B is byte-stable across two turns (U1's test, unchanged). Acceptance: `control-negative-spiderman` and `control-negative-feeling-down` clean 3/3 on the live rerun with median TTFT within 1.25 times the old path's. Out of scope: `remember` (a write, not a duplicate). Exit: `bash scripts/check.sh` and the rerun table.

- [x] **REPLAY-BAR-01: the replay set's bars follow the adopted design, not the legacy corpus** Landed 2026-09-23 (c-99p; the commons corpus row and the replay row both expect the search; commons `spec-v0.1.30`, home's pin bumped). **Narrower than the item's own original scope below**: only the Stephen King flip landed in this pass. The chile row's own turn 3 ("yes") stays where it was (not split out) and `docs/dev.md` wasn't touched - that split is `CONFIRM-01`'s own scope (already its own row, below), not done here; ticking this item only closes the Stephen King half. A review of the landing pass also caught `outcomeArgsMatch` missing its required `packageId` wrapper (every other row in this file has `{ "packageId": "websearch", "args": {...} }`, this one had bare `{ "expression": ... }`), which would have thrown in `conversationScore.ts` on the first live replay run scoring this turn - fixed before push, never shipped broken. (S, Sonnet, 2026-09-23). Objective: `control-negative-stephen-king` ("what's the latest Stephen king novel") expects `toolRan: null` from the tool-call corpus, but under the interim rule it is a world question with a fresh fact, so both paths "over-search" against the wrong bar; flip its expectation to `toolRan: "websearch"` with `outcomeArgsMatch` `expression` `stephen king` and `sourcesNonEmpty`. Files: `backend/scripts/bench/datasets/owner-replay.json`. Acceptance: the fixture's own test passes; the next rerun's tables carry the new bar. Exit: `bash scripts/check.sh`. The same flip lands in commons' spec/llm/tool-call-corpus.json, where the Stephen King row still says expect_calls [] (Fable, 2026-09-23): spec first, one tag, then the pin.

- [ ] **CONFIRM-01: a "yes" to the hub's own offer runs the search the offer named** (S, Sonnet, 2026-09-23; parity failure on both paths, outside the U6 bar). Objective: on `president-of-chile-when-born`, turn 2 ("when was he born") ends with the hub offering to look it up and turn 3 ("yes") runs no search on either path. Files: `backend/src/lib/turnMachine/nodes/policy.ts` (`preConfirmed`), `machine.ts` (the `asked` state and the parked ask). Mirror: the parked-ask path the commands node already takes for a confirm. Tests in these words: after a turn whose reply offered a lookup, "yes" produces the websearch whose `expression` names the person the earlier lookup found (never `he born`), with sources. Acceptance: the chile row's turn 3 clean 3/3 on the live rerun. Exit: `bash scripts/check.sh` and the rerun table.
...
- [x] **U4: the answer register by surface** (M, Sonnet, done 2026-09-23; `f17dcc1b`). RESP-01's own budget half, on the new path (`turnNext.ts`): `surfaceClass.ts` (`SurfaceClass`, `surfaceClassOf`), `register.ts`'s `planFor` takes an optional `surfaceClass` (defaults "spoken" - today's act table, unchanged - so `turnEngine.ts`'s four call sites need no edit), a written turn's budget sized by the evidence (named constants), `planLine`'s written wording; `replyConstraints.ts`'s `constraintsFor` gains an optional `currentTurnId` that decays a length/shape row after its setting turn's own next three done turns (a banned phrase never decays), backward compatible with the frozen path's one no-argument call site. Tests: `register.test.ts`, `replyConstraints.test.ts`, `surfaceClass.test.ts`, `turnNext.test.ts` (a written question's `max_tokens` is the plan's 440, not 120). Bench: `written-set.ts` (twenty typed questions, humanVerdict-judged, run scripted here - a live judge needs an engine this session's environment doesn't have, named rather than faked); the existing offline guards bench reruns clean at its documented 22/29. **Narrowed from RESP-01's own scope (coordinator's ruling, 2026-09-22):** the persona-prompt half (`WRITTEN_POLICY`, `composePersonaPrompt` taking the surface class) is out - the new path's `messages.ts` never calls `composePersonaPrompt` at all today (no persona or policy text reaches its prompt on either register), so wiring one in without the other would be a partial, untested change; named as U4b below instead. Exit: `scripts/check.sh` plus the named benches.

- [ ] **U4b: persona composition on the new path** (S, now measured; the remaining written-class fixes after U4b-2 are PREFIX-CLASS-01's, per PARITY-BISECT-02; dev.md "U6 rerun 2 ruling" (1): with no persona, no naturalness policy and no plan line in the prompt, a plain reply decodes for 6.6 s where the old path's decodes in 0.5 s (`control-negative-dye-hair` r1), the largest plain-turn cost after the forced call; carry the old path's stable prefix (`buildStablePrefix`: the identity line, `composePersonaPrompt`, the information-handling and naturalness policies, about 567 tokens, which also gives NEXT-CACHE-01 a real prefix to protect instead of three tokens) as the stable system message, and the plan line (`register.ts`'s `planLine`, "How to answer this one: ...") as a volatile item before the utterance; acceptance adds: `control-negative-dye-hair` and `control-carrie-series` plain totals within 1.25x or 300 ms of the old path's on the interleaved rerun; **the reply floor (owner's rule, 2026-09-23, the state record "The reply floor"):** the naturalness policy becomes spoken-class only and a written policy is written beside it (structure permitted and expected: lists, headings, steps), the engagement fragment reads the surface class ("brief" on the written class means no padding, never "a sentence or two"), and on the written class `max_tokens` comes from the budget's reply ceiling, never `max_words` times a constant; acceptance adds: on the written set the new path's replies keep the bare reply's points and structure on every adult typed row, the WRITTEN-PARITY-01 judge column read by a person on every failure). `turnMachine/messages.ts` builds the system message from `ContextItem`s only - no persona dimensions, no `NATURALNESS_POLICY`/`WRITTEN_POLICY`, no length-in-prose fragment reaches the model on either register today, unlike the old path's `composePersonaPrompt()`. This is the gap U4 found and did not close (narrowed scope, above): wire persona composition into the new path's own message assembly, surface-class-aware, before `WRITTEN_POLICY`/`composePersonaPrompt(persona, surfaceClass)` from RESP-01's original brief has anywhere real to land. **Landed with a gap (U4b-2, 2026-09-23, `4380798a`/`b5812398`):** all seven layers this row and "The reply floor" name are wired and individually verified correct on the reconstructed prompt (`WRITTEN_POLICY`, the written engagement fragment, the reply ceiling as `max_tokens`, `compositionInstruction`'s split, plus three found live - `FORMALITY_FRAGMENT`'s written twin, the few-shot examples omitted on written, the plan line reworded) - but the holistic bar (the new path's reply carries the bare reply's points and structure) still fails on a live written question, the model stopping on its own well under every token ceiling tried. Not a wiring defect in any named layer; PARITY-BISECT-01 (below) isolates which structural component of the new path's own prompt shape, not its prose, causes the collapse. Row stays open on that verdict. Exit: `scripts/check.sh`.

- [x] **U4c: the plan re-derives from real evidence, not a static zero** (S). `turnNext.ts` computes its one `planFor` call up front, before `context` or any tool round runs, with `evidence` hardcoded to `{ choices: 0, sources: 0, deliverable: false }` - the gap U4 found and named rather than fixed: a written question's evidence-boosted row (360 words / 24 sentences, `register.ts`'s own `writtenQuestionHasRoom`) is unreachable on the new path today, since nothing ever re-derives the plan once real evidence exists (a source came back, a deliverable resolved). Objective: re-derive the plan once real evidence is known - after the tool round, before `answer` - from the actual outcome (sources present, a deliverable outcome, a `pick` choice count), the same `planFor` call, never a second one. Acceptance, in these exact words: a written question whose tool round returns one source gets the 360/24 row and `planLine`'s own text changes with it; a written question with no tool round, or one that returns nothing, keeps the base 220/14 row. Exit: `bash scripts/check.sh`. Landed 2026-09-23 (dev.md "U4c: the plan recomputes from real tool-round evidence"): `machine.ts`'s `derivePlanFromEvidence` action, wired into `tool`'s `onDone`; `choices` stays 0, no real per-outcome signal exists for it yet.
...
- [x] **U6a: the route reads the setting, once - the one boundary between the two paths** (S, Sonnet, done 2026-09-23). Found while building VOICE-LIVE-02 (a real, if narrow, gap: U6's own "default turn.pipeline.next to on" reads as if the live route already honored it, and it never did - only `conversationRunner.ts`'s bench harness read the setting, so the new path had no way to reach a real household turn at all). `routes/turn.ts`'s two turn routes (`POST /`, the `runTurn` call; `POST /stream`, the `runTurnStream`/`runBareTurnStream` call at line ~608) now read `turn.pipeline.next` through the settings registry (one definition, no new key) and call `runTurnNext` when it is on, the frozen path otherwise; `bare` mode stays on the frozen path regardless (a debug bypass, never a household turn); the bench harness reads the identical setting the identical way, so a bench run and a real turn are never on different paths for the same value. `runTurnNext`'s own opts surface is narrower than the frozen path's (no `thinking`, `supersedes`, `continuation`, `ephemeral`, or robot evidence yet - U2's own scope to widen, not this item's). Tests (`turnRouteU6a.test.ts`), in these exact words: with the setting off a turn runs the frozen path, spied by the absence of the new path's own `stats.nodes[]` trace; with it on, the new path, the trace present with all eight node names; the response's own event shape (`turn_meta`, `signal`, `done`, in that order - `POST /stream`'s existing "immediate" branch already handles this generically, since `runTurnNext` only ever resolves "immediate") is identical either way. Exit: `bash scripts/check.sh`.

- [x] **U6: the flip** (S, the coordinator's call on the numbers; gated on ARCH-MEASURE-01's budget records for the models in the house). **Done with an accepted exception (2026-09-24, Fable's ruling on rerun 3): PHRASE-02 (the six cache rows named below) and the scorer fix above; verified at rerun 3, log `data-scratch/rerun-3-proper-20260923-214928.log` on the household's own box. Full account: dev.md "U6: the flip, decided."** `turn.pipeline.next`'s default is `true` (`backend/src/settings/aiKeys.ts`); the old path (`buildSystemPrompt`, `runTurnStream`) stays reachable until the plan's section 2 deletions. The route branch is U6a (done). **Not yet (2026-09-23, rerun 2 interleaved, dev.md "U6 rerun 2 ruling"):** three of five conditions pass; the two fails are design, not the load spike (the spike sat on both halves of its pairs, whose ratios are 0.82x and 1.88x): the forced call's miss (FORCED-CALL-01), the phrasing round's fresh 45-token prompt that also evicts the slot's prefix (PHRASE-01), and the plain reply's unconstrained length (U4b, measured); the 42.22x is a per-turn ratio on a millisecond command pair, so the plain-turn bar gains an allowance (1.25x or within 300 ms) and names its worst pair. Rerun 3 after FORCED-CALL-01, CONTEXT-RECALL-01, PHRASE-01 and U4b, under a hold that stops every process on the box. Earlier verdict (the bar rerun): the France miss was the model node's 20 s deadline delivering an empty reply before the builder fallback exists (DEADLINE-01), the Luna miss a `<function_call>` envelope delivered as text (ENGINE-CONTRACT-03), and the slowdown the new path's own design in three places (thinking on every generation, THINK-DEFAULT-01; no prefix reuse because the per-turn context sits in the first system message, NEXT-CACHE-01; the forced search firing on arithmetic and the clock, SIGNAL-02), with `cached_tokens` unmeasurable until USAGE-01. Flip after those land and one interleaved, trace-kept rerun meets that section's (d). Earlier verdict (2026-09-23, the live rerun): the new path takes 5 of 8 failed rows to the old path's 3 and loses three everyday controls to two integration gaps (an empty-arguments websearch call fed back as a failure, ENGINE-CONTRACT-02's scope; a duplicate `recall` tool in the offered set, TOOLSET-01); the flip's bar is that section's four points, run once after both land: the five clean failed rows stay clean with no engine-classed row, the three controls back to clean with every control's median TTFT within 1.25 times the old path's, the Stephen King control re-bared as a fitting search (REPLAY-BAR-01), the chile "yes" turn its own row outside the bar (CONFIRM-01). Previous verdict (2026-09-22 22:45): U2d's live replay ran the new path under a substring grounding check the record never meant (new path 0 of 7 failed rows and 8 of 13 controls; old path 2 of 6 and 12 of 13); rerun after the term-level fix (2a28e4e8) and decide on the rerun. Default `turn.pipeline.next` to on. Files: the settings registry default. Acceptance: the replay set report on both paths in dev.md, the new path passing every failed row and matching or beating the old path on every control row; the old chat page unchanged. Exit: `scripts/check.sh` plus the replay bench report. Gating: ARCH-MEASURE-01, U2, U3, U4, U5 landed.
...
Every person has their own storage, a parent caps each person and the household, and a person shares a file with named people or the family; an adult may share one file outside the house by a unique link the hub itself serves. The design records are [docs/plans/household-storage-2026-09-23.md](plans/household-storage-2026-09-23.md) (L; its six owner decisions are recorded) and [docs/plans/external-sharing-2026-09-23.md](plans/external-sharing-2026-09-23.md) (L; its numbered questions are the owner's); the spec rows are `STORE-SPEC-01` and `SHARE-LINK-01` in commons.

- [ ] **STORE-CAP-01: the two caps, enforced at the record API in the person's words** (M, Sonnet; after STORE-SPEC-01). Objective: the three settings keys (`storage.household.cap_bytes`, `storage.person.default_cap_bytes`, `storage.person.cap_bytes`) declared once and rendered by the generic renderer; the host's record API refuses a write past the owner's cap or the household total with the reason ("Your storage is full; delete some pictures or ask a parent for more room" for a child, with "or raise the limit under Settings" for an adult); usage per person and per household computed from the records, a shared file counted once against its owner; a reconcile that finds bytes without a record or a record without bytes raises a health item, never a purge. Files: the settings registry (spec first), `backend/src/lib/packageHost.ts` (the record API), a new `backend/src/lib/storage/usage.ts`, the health list. Mirror: the allowance settings and the one health list. Acceptance, in these exact words: a child at the cap writing a picture gets the child line, an adult the adult line, a shared file counts once against its owner, no job ever deletes a file; a picture job whose estimate will not fit is refused before it starts with the same line (decision 3). Out of scope: sharing (STORE-SHARE-01), the page (STORE-PAGE-01). Exit: `bash scripts/check.sh`.

- [ ] **STORE-SHARE-01: sharing a file with named people or the household** (M, Sonnet; after STORE-SPEC-01). Objective: share pointer records (the spec's `share`: file id, from, to a person or the household) created and deleted by the owner only, re-sharing as a further pointer on the same file, never a second file record or a copy of the bytes; a second person adding identical bytes receives a pointer to the first person's file (no duplicates at any level); a shared file appears in each recipient's Library under the owner's name and counts against the owner; a child's sharing bounded by the consent floors (the household and its adults); a package reads a shared file only within its `min_role` and permissions; the turn pipeline's disclosure filter treats a shared file as context with the owner's disclosure. Files: the files routes, the Library page, `backend/src/lib/access.ts`, the context node. Mirror: the memory store's `canRead`. Acceptance: a shared file is listed for the recipient and not for a third person; unsharing removes it at once; a child cannot share outside the household and re-shares only within it, an adult may re-share anything shared with them within the household (decisions 2 and 6). Exit: `bash scripts/check.sh`.

- [ ] **STORE-DELETE-01: a deleted person's files** (S, Sonnet; after STORE-SPEC-01). Objective: the existing person-deletion path (`backend/src/lib/personLifecycle.ts`) gains one step in one place: the person's files with no live share pointer are purged with the person, records and blobs (a blob only when no record points at it); a file with any live share remains, its ownership passes to the household, it counts against the household cap and no personal cap, and the person's name stays in provenance. Mirror: the memory store's tombstones on person deletion. Acceptance, in these exact words: bramble creates a picture and shares it with lucia, lucia shares it with the family, bramble's account is deleted: the picture remains, listed under the household, counted against the household cap, with bramble in its provenance; a second picture bramble never shared is gone, blob and record, after an export was offered; a memorialized person's files all remain (decision 5). Exit: `bash scripts/check.sh`.

- [ ] **SHARE-LINK-02: the link route, the token and the owner's controls** (M, Sonnet; after SHARE-LINK-01 in commons; the design is `docs/plans/external-sharing-2026-09-23.md`). Objective: the one public route as its own mount (no session, no cookie, nothing else reachable), the token hashed with the pepper like a device token and looked up in constant time, an identical not-found for wrong, expired and revoked tokens, expiry and the download limit, revoke as one action, the view log for the owner, a per-address rate limit; the record API refuses `kind: "link"` without `home.public_url` set and for a child, and creating a link is a consequential action with a real confirmation. Files: a new `backend/src/routes/link.ts`, `backend/src/lib/deviceTokens.ts` (the token pattern to mirror), `trustProxy.ts`, `packageHost.ts` (the record API), `secret.ts`. Mirror: the device-token flow and CREDENTIALS.md (scoped, expiring, revocable, never logged). Acceptance, in these exact words: an adult links their own picture and the link opens the file from outside through the proxy; a link created without an expiry choice expires 30 days later and "no expiry" at creation does not; an adult's request to link a file another adult shared with them is refused, and an adult's request to link a child's file shared with them succeeds and is logged as a parental action; the same token after revoke answers exactly as a random token does; a child's request to create a link is refused; a link created without a public address is refused with "This hub has no public address; an adult can set one under Settings"; every open appears in the owner's view log. Out of scope: the viewer page (SHARE-LINK-03), the settings and page controls (SHARE-LINK-04). Exit: `bash scripts/check.sh`.

- [ ] **SHARE-LINK-03: the viewer page** (S, Sonnet; after SHARE-LINK-02). Objective: the plain page a viewer gets: the file shown, played or offered, its kind and size, nothing else; no account, no household or person name, no other file, no link into the hub, no script, no third-party asset; no-store headers on the page and no long-lived cache on the file; the safety floor on the served content the same as in-house. Files: the link route's page from the template's empty-state layout (no hand-built UI). Acceptance: a capture of the page for an image and for a document; a file the household would not be shown is not served; the response carries no cookie and no script. Exit: `bash scripts/check.sh` and the captures.

- [ ] **SHARE-LINK-04: the public address setting and the Storage page's link controls** (S, Sonnet; after SHARE-LINK-02). Objective: `home.public_url` declared once (household, advanced, default unset; the wizard never sets it), the Storage page's link controls from the template (create with the expiry choice defaulting to 30 days and "no expiry" offered, the download limit, the token shown once, revoke, the view log), the privacy page's "what leaves the house" row for links. Files: the settings registry (spec first), the Storage page, the privacy page. Mirror: SETTINGS.md's generic renderer; PRIVACY.md's table. Acceptance: with the address unset the controls are absent; with it set an adult creates, copies once and revokes a link, and the privacy page lists the row. Out of scope: the route (SHARE-LINK-02). Exit: `bash scripts/check.sh` and the captures.
...
## Studio day one (2026-09-23)

A month of lead time is prework time: stage, script and dry-run now so the Studio downloads nothing large, runs a checked runbook, evaluates its own configuration, and becomes a coding lane. The design record is [docs/plans/studio-day-one-2026-09-23.md](plans/studio-day-one-2026-09-23.md); its numbered questions are the owner's.

- [ ] **STUDIO-STAGE-01: the staging kit** (M, Sonnet). Objective: `scripts/studio/stage.sh` fetches every day-one artifact by its pinned URL and verifies its sha256 through the Stack's own download code (`stack/backend/src/lib/download.ts`), writing the Stack's store layout onto an external drive with a `STAGE.md` of every hash and size, and `--verify` re-hashes the drive; `--install <drive>` copies it into place. Items: llama-server b10797 macOS arm64, Qwen3.8-27B and Qwen3.6-35B-A3B as GGUF at Q6 at pinned revisions, the two Splash packages (17.4 GB and 20.9 GB, pre-seeded into the Hugging Face cache layout; the package is proprietary, 4-bit weights with the vision encoder and the draft in one file that only Splash loads, so the GGUF cannot stand in for it and both forms are staged on purpose), the MLX-format weights of both candidates at pinned revisions for oMLX and mlx-serve, the pins the installer and the Stack already carry. Acceptance: `--verify` green on this Mac's drive; a dry run proves whether `splash serve` honours the pre-seeded cache with the network denied, recorded either way; the Splash formula revision in `STAGE.md` (the Studio ships with macOS 26.4, decision 1 of the record, so no macOS contingency). Out of scope: any model not in MEASURE-02. Exit: `bash scripts/check.sh` and `STAGE.md`.

- [ ] **STUDIO-RUNBOOK-01: the runbook, unboxing to the acceptance workload** (M, Sonnet). Objective: the record's eleven steps as a checked document (`docs/dev/studio-runbook.md`), each with its command and its check, a target under two hours; every scriptable step gains a `--dry-run` flag (`scripts/install.sh` has one) and is run on this Mac now with its output recorded. Acceptance: the dry run of every scriptable step recorded in dev.md; the restore step rehearsed against a fresh data directory from this Mac's latest backup. Out of scope: the evaluation (STUDIO-EVAL-01). Exit: `bash scripts/check.sh --docs` and the recorded dry run.

- [ ] **STUDIO-EVAL-01: the Studio evaluates its own configuration** (M, Sonnet). Objective: `scripts/studio/evaluate.sh` chains, on a scratch data directory, the Stack's Studio bench protocol (`stack/scripts/bench/studio-bench.sh`), ENGINE-CONTRACT-01's suite per engine (the set is four: llama-server, oMLX, mlx-serve and Splash, each through the same suite, the same MEASURE-02 rows and the same workload; the summary picks by the numbers and the contract, never by a headline claim, and an engine that loses on chat keeps its own role: oMLX's SSD prefix cache for long companion contexts, mlx-serve's media roles), ENGINE-SPLASH-01, MEASURE-02's rows per candidate with llama-server run with the model's MTP head on and every latency measured cold and warm at 32K with the effective rate from send to last token, and STUDIO-ACCEPT-01's workload with first useful answer cold and warm, collecting the summary rows into one `report.md` in the protocol's shape and appending them to dev.md "Measured so far", so a Claude session reads tables. Mirror: `studio-bench.sh`. Acceptance: a rehearsal on this Mac (tier 1, the 8B) runs end to end and writes the report with the rehearsal marked as such; the report's summary names the candidate that passes the contract suite, clears the tool-calling bars and gives the best first useful answer under the workload. Exit: `bash scripts/check.sh` and the rehearsal report.

- [ ] **STUDIO-LANE-01: the Studio as a coding lane** (M, Sonnet; after STUDIO-EVAL-01). Objective: the coordinator's OpenCode HTTP driver pointed at the Studio's engine, with the lane rules from the record: a fresh session and a fresh worktree per brief at the base the coordinator names, deleted after the report; a wall-clock budget (three times a reference run) in place of a step count and a run that ends without a done report counted as blocked; the tool set the brief names (read, edit, the named tests, commit, never push); S and M coding items, second reviews by a different model, the benches. `splash claude` proven on the Studio before Claude Code is routed to it (Splash's README ships the launcher and names its Anthropic Messages endpoint; it never names Claude Code). Acceptance: one S item from the backlog done end to end on the Studio with a clean done report and a review by the other model; the driver's budget stop tested with a brief that cannot finish. Out of scope: anything the record keeps on the subscription (design, verdicts, diagnosis, cross-subsystem items, the first review of a safety change). Exit: `bash scripts/check.sh` and the done report.

- [ ] **STUDIO-MOVE-01: the Studio beside this Mac, then the move, then the rollback** (S, Sonnet). Objective: this Mac's Home points its chat, embed and judge roles at the Studio's engines through the Stack's URL tier over the LAN (the privacy page's row unchanged: in-house); then Home moves per the runbook's step 11 with the Studio restoring from this Mac's last backup and this Mac's hub stopped, never deleted; rollback is repointing the URLs or restarting this Mac's hub on its untouched data directory. Acceptance: a turn on this Mac's hub answered by the Studio's engine; the rollback rehearsed. Exit: `bash scripts/check.sh --docs` and the recorded rehearsal.
...
- [x] **VOICE-LIVE-01b: the overview route reports Home's own supervisors when no Stack is configured** (S, route + guard) - **landed 2026-09-23.** Live finding, the day VOICE-LIVE-01 landed: Jesse reloaded 8787 (no Stack) and saw no waveform button. Cause: `GET /api/engines`'s overview route returned `roles: []` unconditionally whenever `isStackConfigured()` was false (`routes/engines.ts`), so `readyRole()` (`frontend/src/apps/chat/engineRoles.ts`) could never see stt/tts as ready on a non-Stack hub no matter how healthy they really were - the same gate also silently hides `CreateImageItem` in `composerAddMenu.tsx`. VOICE-LIVE-01's own acceptance passed on a mocked overview, so nothing caught it. Fix: `backend/src/lib/homeSupervisorRoles.ts` (new), one `RoleInfo`-shaped read per Home supervisor (chat/llmSupervisor, tts/ttsSupervisor, embed/embedSupervisor, stt/the stt module's own asset check, image/honestly not implemented) - chat/tts/embed report `ready` on the stub-fallback reasoning every one of their own supervisors already guarantees (their own header comments: nothing selected/configured still bottoms out in a real, working in-process stub, never a failure), stt reports `ready` only when its real assets are actually on disk (no stub exists for it), image stays `notInstalled` (nothing built there yet). Wired into the overview route's existing no-Stack branch; the Stack-configured path is untouched. Acceptance (exact words): with no Stack configured, the STT assets installed and the TTS backend startable, GET the overview lists stt and tts as ready and `/next/chat` renders the waveform button - verified with a route test using the real handlers (no mocked overview): `backend/tests/enginesRoutes.test.ts`'s two new/updated tests (the default fresh-install state, stt notInstalled/chat+tts+embed ready/image notInstalled; then with real STT asset files staged, stt ready). With the Stack configured the route's output is unchanged (existing test, untouched). Exit: `bash scripts/check.sh` green.

- [x] **VOICE-LIVE-01: the composer's right-side slot so the waveform mounts** (S, kit, upstream-bound) - **landed 2026-09-23.** Objective: `ComposerAction`'s right group gains an append point (`ComposerExtraEnd` on `ThreadComponents`) so the built waveform button and chevron (`frontend/src/apps/chat/composerVoiceControls.tsx`) mount. Files: `commons/ui/src/elements/thread.aui.tsx`'s composer (`ui-v0.5.36`), the kit's upstream note (`ui/docs/dashboard-upstream.md`, also corrected the first five slots' stale "PR not yet opened" status to the real open PR). Mirror: the left slot `ComposerExtra`, added upstream onto assistant-ui/assistant-ui#8003 as a sixth append point the same day. Acceptance: the waveform renders in the composer on `/next/chat` when stt and tts are both ready and is absent otherwise (verified: NextChatPage.test.tsx's VOICE-LIVE-01 describe, both states, plus composerVoiceControls.test.tsx's own three tests); the PR link in the pending-patches table (done). Out of scope: the live session (VOICE-LIVE-02). Exit: the kit's tag pinned in Home (`ui-v0.5.36`) and `bash scripts/check.sh` green.

- [x] **VOICE-LIVE-02: the live voice session** (M, Sonnet; the plan's section 1) - **landed 2026-09-23 (`906d740b`), measured 2026-09-23.** Objective: press the waveform and talk: the shipped `voice-conversation.tsx` Element shows listening, the Silero detector ends the utterance, the `stt` role transcribes, the turn runs with `spoken: true` (RESP-01's flag), `reply.speech` is spoken sentence by sentence as it streams through the `tts` role and the spec's sentence chunker, the Element shows speaking, the transcript lands in the thread as text, and stop works at any point from the Element. The visual feedback is the Element's own rings, which it already scales from its `amplitude` prop (0 to 1): the level comes from a Web Audio `AnalyserNode` on the microphone stream, computed in the browser and sent nowhere (the privacy-safe visualizer the owner asked for, 2026-09-23; the browser's speech recognition is never used, see VOICE-LIVE-03), the same meter driving the Element while listening and the reply's audio while speaking; no third-party visualizer, and if a bar-style waveform is ever wanted, `react-audio-visualize` (MIT, an `AnalyserNode` over a `MediaRecorder`, no network) is the maintained option to take through the package manager, never a copied one. Files: `frontend/src/apps/chat/` (the runtime adapter, `composerVoiceControls.tsx`), `backend/src/lib/stt.ts`, `sttSession.ts`, `tts.ts`, `backend/src/routes/turn.ts` (the `spoken` flag). Mirror: the dictation path and the read-aloud Element's own stop. Acceptance: a spoken turn end to end on this Mac with the first spoken word under 3 seconds after the utterance ends beside the resident 8B, measured and in dev.md; a child's turn keeps the child rules and carries no reasoning; no new model, no new settings key. Out of scope: barge-in (the existing `VOICE-01`, which follows this item and depends on its stop control and abort signal), the wake word (HANDSFREE-01 c). Exit: `bash scripts/check.sh` and the measured line. **Measured 2026-09-23 (dev.md "VOICE-LIVE-02: the measured line"):** the household's real default (`turn.pipeline.next` off, today's engine) clears the bar with room to spare - median 1.40s, min 1.07s across 5 reps. The rebuilt engine (`turn.pipeline.next` on) does not - median 6.21s, min 4.75s, every one of 5 reps over the bar. This item's own acceptance is met for the shipping default; the rebuilt engine's own voice-turn latency is a separate, real gap, named for whoever owns that pipeline's speed next (GATE-SPEED-02).

- [x] **VOICE-LIVE-03: the chevron's voice selection and microphone picker** (S, Sonnet) - **landed 2026-09-23.** Objective: the waveform's chevron lists the `tts` role's voices from the voice catalog, lists the microphones the browser reports (MediaDevices `enumerateDevices()` after permission, names shown without hardware ids, refreshed on `devicechange`) so a person with more than one input picks one. The microphone choice is a per-browser preference (`frontend/src/lib/voice/micDevicePreference.ts`, the same class as a remembered tab), never a household setting; the dictation adapter opens the chosen device (`mic-capture.ts`'s new `deviceId` option, `{ exact: deviceId }` so a stale id fails loudly rather than silently falling back). Built from the kit's own primitives (the menu the chevron already opens); RESP-04 (f)'s ruling applied (`MicrophoneGroup` renders nothing at fewer than two devices). Acceptance met: choosing a voice changes the next spoken reply (`VoiceChevron` writes the real `tts.voice_id` setting through `POST /api/voice/catalog/select`, reads it back on mount so the choice survives a remount); with two fake input devices the menu lists both and choosing the second makes the next dictation open that device id (`composerVoiceControls.test.tsx`'s own VOICE-LIVE-03 test, through the fake `navigator.mediaDevices`). The wake-word shortcut clause is deferred, not silently dropped: `composerVoiceControls.tsx`'s own header comment traces why - HANDSFREE-01 (c) (a device's wake-word setting key) doesn't exist yet, and no "wakeword package installed" concept exists either (only a Stack engine role of that name, a different thing), so there is nothing yet for a shortcut to point at; the file implements the honest absent case today and HANDSFREE-01 (c) is the item that builds the key this menu would then read. Out of scope (unchanged): the wake word itself, shadcn.io's "speech-input" button (rejected 2026-09-23, sends audio to the browser vendor's servers). Exit: `bash scripts/check.sh` green.
- [x] **VOICE-LIVE-03b: the chevron leaves the composer; voice selection moves to Settings** (S, review low) - **landed 2026-09-23.** Owner's live read of 7d3f83d7 on 8787 (screenshots): the composer's dark-circle waveform opened a chevron listing raw catalog file names ("zerocool_enhanced.wav.1e68beda@240.safetensors") under "Voice-donations" - his ruling, "ChatGPT has character voice selection in Settings, not out on the prompt bar." Objective (1) the composer's trailing slot holds only the waveform button now, styled as a filled pill (the kit's shipped `size="icon"` Button, `rounded-full bg-primary`, lucide's `audio-waveform` icon added to the kit's curated set, commons `ui-v0.5.37`) instead of the `VoiceOrb` glyph; (2) voice selection and the microphone group both moved into Settings > Voice's existing `VoiceCatalogSection.tsx` as two controls in the same section, the same code paths (`api.selectVoiceFromCatalog()`, `micDevicePreference.ts`), with a line noting the microphone choice is remembered on this device only; (3) a new `frontend/src/lib/voice/voiceDisplayName.ts` derives a person-readable name from the file stem (strips a trailing real extension, a trailing content-hash segment some donated files carry, and a second extension if the hash sat between two, then turns underscores into spaces) since `VoiceCatalogEntry` (`backend/src/lib/voiceCatalog.ts`) carries no name/title field at all, confirmed against the Hugging Face tree API's own response shape - a real catalog gap, not invented around; (4) VOICE-LIVE-02's live session gets its own settings-gear entry point into Settings > Voice, Session B's own scope, notified once this landed. **Correction to the brief:** "until VOICE-LIVE-02 lands its click does nothing, disable it" no longer applied - VOICE-LIVE-02 had already landed (`906d740b`) by the time this item started, confirmed by reading the current `composerVoiceControls.tsx` before touching it, so the pill keeps its real `onClick` (opens the live session via `voiceSessionContext.tsx`), never disabled. Acceptance (exact words): on `/next/chat` the trailing slot shows one waveform pill and no chevron, captured at 1440 and 390 light and dark; Settings > Voice shows the voice list with readable names and the microphone control, captured; choosing a voice there changes the next spoken reply (the existing test moves with the control). Exit: `bash scripts/check.sh` green and the captures opened.
- [x] **VOICE-LIVE-04: the composer shows a live waveform while dictating, as ChatGPT does** (S, Sonnet; owner's ask 2026-09-23, after VOICE-LIVE-02) - **landed 2026-09-23 (`e0ceffc7`).** Objective: while dictation records, the composer's text area gives way to a live bar waveform of the microphone (ChatGPT's dictation look), drawn in the browser from the same local level source as VOICE-LIVE-02 and sent nowhere; the bars stop and the transcript replaces them when recording ends. Prebuilt over hand-built: the row's own first plan named `react-audio-visualize`'s `LiveAudioVisualizer` - verified broken instead (a bare import crashes under this stack's React 19; see dev.md), so this uses the native `AnalyserNode` through `audioLevelMeter.ts` (VOICE-LIVE-02's own proven mechanism) into a rolling window of recent levels drawn as bars, colored from the design token. Files: `frontend/src/lib/voice/sttDictationAdapter.ts`, `frontend/src/apps/chat/composerDictationWaveform.tsx`, `commons` `ui/src/elements/thread.aui.tsx` (`ComposerInputOverride`, ui-v0.5.40 - see dev.md, the composer this feature actually needed to reach lives in the vendored kit, not this repo's own `apps/chat/thread.aui.tsx`). Mirror: the composer's existing dictation state. Acceptance: a headless run with the fake media stream shows moving bars during recording and the transcript after (captured, both bugs found and fixed before landing - see dev.md), light and dark at 1440 and 390; the network log shows no request carrying audio. Exit: `bash scripts/check.sh` and the captures opened.
- [ ] **VOICE-LIVE-05: the voice mode looks like ChatGPT's, with a little flair** (S, Sonnet; owner's ask 2026-09-23, after VOICE-LIVE-02) - **landed 2026-09-23 (`0241b703`), Jesse's own judgment of the recording still owed.** Objective: the live voice session's centerpiece is an orb the size ChatGPT's is, breathing with the local level from VOICE-LIVE-02: the shipped Element's rings as they ship, restyled only through the tokens they read (a gradient and a soft glow per mode: listening, thinking, speaking, with the speaking color a token rather than the Element's hard-coded blue if that line is ours to override through a token, otherwise noted as an upstream ask), and one flair touch that costs nothing in latency (a slow hue drift while thinking, a ripple on the first spoken word), judged from a short recording, not a still. No hand-built orb: if the Element's rings cannot reach the look, the gap is named in the design record first and a maintained orb (a canvas library through the package manager, MIT, local only) is the next candidate, decided before a line is written. Acceptance: a recording of listen, think and speak on this Mac opened and judged by Jesse, plus the four captures; the level animation costs under 2 ms a frame (measured). Exit: `bash scripts/check.sh`, the recording and the captures. **Landed:** the speaking color was ours to override - `bg-blue-500`/`dark:bg-blue-400` were literal utilities reading no token at all (commons owns this Element, no upstream assistant-ui import in it), fixed at the source to `bg-primary` (one token, not two hard-coded shades - see dev.md). Both flair touches are CSS-only (`@keyframes`, no rAF loop). The gear into Settings > Voice rides along, the same file. Per-frame cost measured twice (two independent runs), both showing added cost within measurement noise of zero (well under 2ms) - method and numbers in dev.md. **Round 2 (2026-09-23, Jesse's own live read of round 1):** too small and, in light theme, near-black (the cascade bug TOKENS-PRIMARY-01 now tracks generally; fixed here with a dedicated `--voice-accent` token) - the orb is ChatGPT's own scale now (160px mobile, 192px desktop); "the phone call look instead of the visualizer" - no transcript list or "you"/"ai" labels (removed outright, not hidden), the orb centered above the controls, mute/end pinned to the bottom as the kit's own `TooltipIconButton`, the surface growing to fill the phone's own screen below `sm` (a real call screen, not a floating card there); the composer's own waveform pill resized to match Dictate/Send exactly (`size-7`, not the 48px floor). Commons `a790400`, home `e25a836a`. The round-2 recording and four captures are versioned `-v2` in the same Desktop folder as round 1, sent to Jesse directly; this row still ticks once he's opened and judged one of the two rounds.
...
- [ ] **PKG-UNINSTALL-01: the normal package uninstall path** (S, Sonnet; before CAP-GATE-01). Objective: the hub has no uninstall path today (`packageHost.ts` and the packages routes have none): add the route and the package's uninstall hook to PACKAGES.md's rule "Uninstall, and a person's files": the package's code removed and its surfaces hidden, its own settings and state kept for a reinstall, the person's media records (commons MEDIA-RECORD-01) untouched and still reachable from Library and Files, the confirmation in the standard's words. Files: the install lives in `backend/src/routes/plugins.ts` and `backend/src/routes/store.ts` (there is no `routes/packages.ts`; pointer corrected 2026-09-23), `backend/src/lib/plugins.ts`, `backend/src/lib/packageHost.ts` only if a host hook is needed, the apps page's uninstall action from the template's menus. Mirror: the install route in `store.ts`. Acceptance: an installed package is uninstalled through the route, its hook ran (a spy), it is gone from the packages route and the apps page, its settings and state rows remain and a reinstall reads them, a media record it wrote is still listed for its owner, and the confirmation text is the standard's; a child profile cannot uninstall. Out of scope: reconcile (CAP-GATE-01). Exit: `bash scripts/check.sh`.
...
- [ ] **ENGINE-SPLASH-01: measure Inco Splash as the tier 3 chat engine** (S, bench, Sonnet, 2026-09-23; from the owner's note). Objective: Splash (github.com/incoai/splash, Apache-2.0; Apple M3 or newer, macOS 26.4 or later, 36 GB of unified memory at least, so tier 3 only, never tier 1, tier 2 or the floor) serves exactly MEASURE-02's two Studio candidates (Qwen3.8-27B and Qwen3.6-35B-A3B, each packaged with a DFlash 2 draft model for speculative decoding and the vision encoder) and claims about twice oMLX's decode speed on the 27B (74 tok/s short, 54 at 32K on a 48 GB M5 Pro) and close to four times under four concurrent prompts, with OpenAI Chat and Responses and Anthropic Messages endpoints, streaming, tool calls, JSON Schema output, images and reasoning_effort levels. The owner's saved transcript of a hands-on Splash review is at `data-scratch/splash/transcript-qwen38-27b-2x-faster-on-mac.txt` (git-ignored, on the dev Mac), and the session that runs this row reads it first for the claims to test. **What the review adds to this row (claims, not our record):** the baseline is llama-server at the pin with Qwen3.8-27B's own multi-token-prediction head on at draft depth two and three, never plain autoregressive decoding, because the reviewer shows Splash's 2x is its DFlash 2 draft against oMLX without one; measure cold and warm at 32K and the effective rate from send to last token (the review cites 96 s cold and 282 ms warm to first token on a 48 GB M5 Pro); include chat-shaped rows beside coding rows (the 2x was on a coding workload with reasoning on); test the 16-concurrent-32K claim against the workload's two conversations; and test Splash's refuse-to-boot memory report beside the governor's admission on the same machine, the governor staying the one authority. Nothing leans on it before it is measured: run ENGINE-CONTRACT-01's suite against it on the Studio (tool_choice required and auto, reasoning separated from content, the thinking limit, cancellation reaching the engine, timing fields), then MEASURE-02's rows and STUDIO-ACCEPT-01's workload with it beside the pinned llama-server build, recording tok/s at short, 8K and 32K context, peak memory and the concurrency result with the engine build and model digest. Files: `stack/backend/src/engineCatalog.ts` (a candidate entry, never a pin), `docs/dev.md`. Mirror: the engine catalog's existing candidate rows. Acceptance: the contract suite's pass or fail table and the numbers in dev.md; a pin only if it passes the contract and beats the pinned build on the acceptance workload. Out of scope: any code path that assumes it; the coding lane (its own decision on the Studio). Exit: the dev.md tables.
...
- [ ] **SHARE-CONV-01: a real Share entry in the conversation menu** (M). CHAT-HEADER-01 originally put a Share item in the conversation menu (`frontend/src/apps/chat/chatHeaderBar.tsx`), disabled, behind `shareAllowed`, since no share-creation route existed yet; CHAT-FIND-0923-03 (2026-09-23) removed it outright (a disabled placeholder is worse than no entry, Jesse's own finding that the menu should hold only real conversation actions) - `shareAllowed` is gone from `ChatHeaderData` too. This item is: a route that snapshots a conversation into a shareable record, the link surface, and a NEW `onShare`/`shareAllowed` pair added back to `ChatHeaderData` (mirroring `onRename`/`onDelete`'s own shape) once share is real, not restoring the disabled item. The shipped `shared-conversation.tsx` Element renders a share once one exists. Depends on: the shipped Element's own data contract (read it first, don't invent a shape). Exit: `bash scripts/check.sh` and the captures.

- [ ] **CONV-ARCHIVE-01: a real archive, shared by both conversation menus** (M, spec first; coordinator's own row, 2026-09-23, not yet picked up). The shipped `elements/thread-list.aui.tsx`'s own three-dots menu always offers Archive (Rename/Archive/Delete, no capability flag to hide one), but `chatThreadListAdapter.ts`'s own `archive()`/`unarchive()` throw unconditionally by deliberate design ("the shared record has no archive state. Do not expose an action that silently deletes a conversation or disappears on reload") - a real, pre-existing gap, not a hand-built limitation to route around. This item closes it: the conversation record gains `archived_at` in `@maipai/spec` (spec first, the shared record shapes go through the spec before either client), the adapter's `archive()`/`unarchive()` set/clear it instead of throwing, an archived conversation leaves the live thread list into the shipped Element's own archived section (it has one - verify before assuming) with an unarchive control, and then both the thread row's own menu and the header's own conversation-actions menu (CHAT-FIND-0923-03) carry a real, working Archive - the header's own item drawn from the one real capability, not a second copy. Acceptance: archiving a conversation from either menu removes it from the live list and shows it in the archived section; unarchiving from there restores it to the live list; a child's own conversations archive/unarchive the same way (no role gate named otherwise); the header's own Archive item does the same real thing the row's own does. Exit: `bash scripts/check.sh` and the captures.
...
- [ ] **EVAL-03: Control vector for register** (S-M; gains the 2026-09-23 motivation below, merged from `PERSONA-STEER-01`, TRUEUP-01's own commit). Retrain from the selected companion's own examples; replaces `NATURALNESS_POLICY` only if the naturalness bench holds and prompt tokens fall. **2026-09-23 motivation (dev.md "The written prompt on tier 1, decided"):** `WRITTEN_VOICE_PROSE = false` (`persona.ts`) took the written-adult prompt's persona and policy prose out entirely, because every measured shape carrying it on tier 1 cost 0.12x-0.48x of the bare floor while the two that carried none (the ceiling, arm e) cleared close to it - a real prebuilt mechanism for varying the written reply's voice (formality, complexity, engagement, filler) that spends no prompt tokens doing it (the engine's own native activation steering, per org principle 6, "prebuilt over hand-built": a paragraph of personality prose instead of activation steering is hand-built, and loses to the prebuilt answer on the same terms) is this item's own case for existing on the written class too, not spoken alone. Needs its own design pass before a line is written: which dial(s) a control vector can actually steer on this model family, how a vector is trained or sourced, how `persona.ts`'s existing four-dial `Persona` shape maps onto it (a new field per dial, a single blended vector, dial values as vector-strength presets), and how it composes with `WRITTEN_VOICE_PROSE`'s own off switch (a real steering path presumably retires the constant rather than sitting beside it). Out of scope until the design pass names it: any specific vector format, training data, or llama-server API detail. **The isolated-ceiling gap (PHRASE-01, dev.md "PHRASE-01: the phrasing round continues the forced call's own prompt", not measured further on tier 1):** the gap between the isolated ceiling (0.51x/0.53x) and the same composition through the real path (0.30x/0.26x) is still open - candidates are the tool block, the profile and roster lines, and `identityLine`'s own wording; this design pass is where it gets a real answer, not a further tier-1 measurement. Exit: a design note in dev.md naming the mechanism and the file/API surface, before any code.

- [ ] **WRITTEN-VOICE-TIER-01: WRITTEN_VOICE_PROSE becomes hardware-tier-aware** (S, 2026-09-23; dev.md "The written prompt on tier 1, decided"; re-sequenced after `EVAL-03`, TRUEUP-01's own commit - the designed fallback is offered per tier only once the primary, the control vector, has been tested, owner's rule 2026-09-23 evening). Objective: `persona.ts`'s `WRITTEN_VOICE_PROSE` is a plain module constant (`false`), not a settings key, because tier 1's own 8B measured every voice-prose shape under the bar - a bigger model (tier 3, the Studio) may not have the same collapse, and this constant is the one place that decides whether the written class's own dial/policy prose (already declared, already tested) ever composes into a real prompt. Change: once STUDIO-ACCEPT-01 or an equivalent tier-3 workload measures the written voice twins (`FORMALITY_FRAGMENT_WRITTEN` and the rest, already exported) clearing WRITTEN-PARITY-01's own bar on that hardware, `WRITTEN_VOICE_PROSE` becomes a value the hardware probe proposes per tier (hardware-tiers-2026-09-23.md's own "the configuration is proposed, never fixed" pattern) rather than a hardcoded `false` - still no settings key, a probed default a person could override the same way any other tier default can be. Out of scope until tier 3 is measured: the probe wiring itself. Exit: the tier-3 measurement recorded in dev.md, then the constant's own change plus a test that tier 1 still defaults to off.
...
- [x] **PERSIST-STRUCTURED-01: the weather/almanac card survives a reload** (S, Sonnet, 2026-09-23; closes getmaipai/home#130) - **landed 2026-09-23.** SHELL-02 slice 3 found this real: `structured_part` (`lib/composer.ts`'s `structuredPartForOutcomes()`) was computed fresh on every live `done` event but never persisted, so a reloaded conversation showed only the reply text, the card gone. Fixed: a new nullable `structured_part` text column on `conversation_turns` (migration 0062, schema version 42 to 43), written in `conversationHistory.ts`'s `buildTurnRow()` from `value.structured_part` beside `reasoning`/`outcomes`, read back with no age gate in both `listConversationTurns()` and `list()` (the card is the reply itself, not the reasoning behind it - REASONING-03's minor-only drop never applied to it). `wire.ts`'s `ConversationTurnWithMemoryIds` omits the raw column and adds a typed `structured_part?: StructuredPart`, the same shape `artifact` already gets. `chatHistoryAdapter.ts`'s reload path rebuilds the identical tool-call part `chatModelAdapter.ts` builds live, in the same order: reasoning, structured, artifact, text, sources. Regression tests in the issue's own words (a turn with a structured part is read back with it after a fresh load of the conversation) in both `conversationHistory.test.ts` (backend, plus a minor-speaker case proving no age gate) and `chatHistoryAdapter.test.ts` (frontend, part ordering). **Live on 8787:** a real "what's the weather in Seattle" turn on the OLD path (`turn.pipeline.next` off - "today's engine," per issue #130's own scope) rendered the weather card, and it survived a full page reload, unchanged. Found live and filed rather than fixed here (getmaipai/home#147): the NEW path (`turnMachine`/`turnNext.ts`) never sets `TurnValue.structured_part` at all - `structuredPartForOutcomes()` is called only from `turnEngine.ts`'s `logTurnSafely()` (the old path) - so a weather/almanac question on the new path shows no card live, before this item's own persistence question ever applies; this item's fix is correct and proven for the path that produces the card today. Exit: `bash scripts/check.sh`.
