...
- [ ] **CHAT-FIND-0923-04: a sent message's bubble must not resize on hover** (S) - **fix landed in commons (`ui-v0.5.47`), home's own pin bump still owed.** Hovering a sent message ("hi") makes its bubble grow wider and the text jump to the left (screenshots: at rest a tight pill around "hi"; on hover the bubble widens to the action bar's width with "hi" left-aligned). Root cause: the bubble and the action-bar wrapper are siblings inside one parent with no width of its own, sized by CSS Grid's own `auto` column - the action bar genuinely unmounts at rest (only the bubble counts then, a tight pill) and mounts on hover (both count, and the wider one wins the column's width, stretching the bubble along). Fixed with `w-fit` on `.aui-user-message-content` (`elements/thread.aui.tsx`) - still bounded by the row's own available width, so a genuinely long message wraps exactly as before. Reviewed medium, zero findings. Blocked on landing here only by an unrelated pin conflict (ui-v0.5.46's own voice-orb work removed `VoiceTurn` from `voice-conversation.js`, which `liveVoiceSession.tsx` needed updating for first - Session B's own item, landed separately) - not this item's own scope. Acceptance: "the bubble's width is the same before and during hover (a computed-width assertion in the page test), captured hover and rest at 1440" - happy-dom computes no real box layout, so this is a real headless Playwright check (`scripts/screenshot.ts`, new flag), not a jsdom unit test, matching how every other pixel-level claim in this batch is proven. Exit: `bash scripts/check.sh` and Jesse's own reload.
