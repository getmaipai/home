...
- [ ] **SHARE-LINK-02: the link route, the token and the owner's controls** (M, Sonnet; after SHARE-LINK-01 in commons; the design is `docs/plans/external-sharing-2026-09-23.md`). Objective: the one public route as its own mount (no session, no cookie, nothing else reachable), the token hashed with the pepper like a device token and looked up in constant time, an identical not-found for wrong, expired and revoked tokens, expiry and the download limit, revoke as one action, the view log for the owner, a per-address rate limit; the record API refuses `kind: "link"` without `home.public_url` set and for a child, and creating a link is a consequential action with a real confirmation. Files: a new `backend/src/routes/link.ts`, `backend/src/lib/deviceTokens.ts` (the token pattern to mirror), `trustProxy.ts`, `packageHost.ts` (the record API), `secret.ts`. Mirror: the device-token flow and CREDENTIALS.md (scoped, expiring, revocable, never logged). Acceptance, in these exact words: an adult links their own picture and the link opens the file from outside through the proxy; the same token after revoke answers exactly as a random token does; a child's request to create a link is refused; a link created without a public address is refused with "This hub has no public address; an adult can set one under Settings"; every open appears in the owner's view log. Out of scope: the viewer page (SHARE-LINK-03), the settings and page controls (SHARE-LINK-04). Exit: `bash scripts/check.sh`.
