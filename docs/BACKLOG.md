...
- [ ] **ROUTE-FIND-03: two live routing failures on the typed screen, 2026-09-22** (M, corpus first). Findings from the owner's own turns on /next/chat: (a) "what did Apple announce this week" got the stuck fallback ("I'm stuck on that one, sorry. Ask me again some other way?") instead of a web search; a question about recent public events is a search, and the fallthrough for a question the model cannot answer should attempt the websearch package before giving up. (b) "search who won the Seattle Mariners game yesterday" got "Noted.", the remember package's acknowledgment: `search <anything>` matched nothing in websearch's patterns (`search the web for *`, `search online for *`) and fell to remember on the bare verb; a leading "search" (and "look up", "google") is an explicit websearch command whatever follows. Acceptance: both utterances in the routing corpus (`backend/scripts/bench/datasets`, the routing rows) with the expected route `websearch`, plus five paraphrases each (a recent-events question without the word search; "search"/"look up"/"google" followed by a plain query), the corpus run green with no regression on the existing rows, and the memory judge never treating a search request as a fact to save (a test in `memoryJudge.test.ts`). Files: `backend/packages/websearch/manifest.json` (patterns), `backend/src/lib/routing.ts` (the recent-events signal, the command verbs), `backend/src/lib/turnEngine.ts` (the fallthrough order: search before the stuck line). Exit: `bash scripts/check.sh` and the routing corpus bench.

- [ ] **TEMP-CHAT-01: a temporary chat that is never remembered** (M, backend first). ChatGPT's temporary chat, and on a household hub whose whole premise is that it remembers, it matters more than it does there: a parent must be able to ask something without it entering history, the thread list, or memory. Not a UI feature - a turn-pipeline one. A conversation marked temporary writes no `conversations` row and no `conversation_turns` rows, and the memory judge never runs on its turns (`backend/src/lib/` memory extraction path); it is gone when the tab closes. **Safety is never skipped**: the safety-first order in `turnEngine.ts`, the role floors and the guards all run exactly as they do on a normal turn (`.github/docs/SAFETY.md`: child-safety protections are non-removable architecture, not a setting), and a child's temporary chat is still age-gated and still subject to every content ceiling. What a temporary chat skips is persistence and memory, nothing else. Entry point: the conversation header's menu (CHAT-HEADER-01) and a New Thread variant, both labelled so a parent understands what is and is not kept. Acceptance: a temporary turn leaves no row in either table (asserted directly against the db), the memory judge is never invoked (asserted with a spy), the thread list does not show it, a child's temporary turn still hits the same safety path as a normal one (a test that a blocked utterance is still blocked), and reloading loses it. Out of scope: exporting one before it is discarded. Exit: `bash scripts/check.sh`.
...
- [ ] **HANDSFREE-01: hands-free replies, on the control the composer already has** (M). The owner wants hands-free - replies spoken aloud without a click, and a wake word - and guessed right that it belongs on the composer rather than as new chrome. It does: SHELL-02 slice 6 already puts a mic (dictation) and a waveform-with-chevron (live voice) at the composer's right, and this rides the chevron's menu rather than adding a control. Three entries: **Read replies aloud** (a persistent toggle, not a mode - the shipped `read-aloud.tsx` Element does the speaking, with its own play/pause, rate and word highlighting, driven by the tts role); **Voice** (the voices route, as slice 6 already plans); and **Listen for the wake word**, shown only when a wakeword package is installed from the catalog (`wakewords/` there), which lets a person start a turn without touching anything. The toggle is remembered per person with the conversation, so it depends on PERSIST-CONV-01; until that lands it is per-conversation-in-memory and the row says so. Every entry hides when its role is unavailable, per the same rule as slice 6, so an unconfigured hub shows none of it. Acceptance: with tts available, a typed turn's reply is spoken end to end and the Element's controls work mid-speech; toggling off stops immediately and stays off for the next turn; the wake-word entry is absent with no wakeword package installed and starts a turn when one is; a child's turn speaks only content that passed the same safety path. Out of scope: barge-in (interrupting the spoken reply by talking over it), which is a robot concern and gets its own row. Exit: `bash scripts/check.sh` and the captures.

- [ ] **MODEL-SEL-01: choosing the model a turn runs on** (M, backend first). Nothing selects a model today: `frontend/src/lib/api.ts`'s `streamTurn` options carry `thinking?: boolean` and no `model`, `backend/src/routes/turn.ts` (both body copies) the same, and there is no engine-selection-by-model path downstream. So the composer control ChatGPT has cannot be built as a frontend slice; the capability has to exist first. Order: the turn route's body schema and the wire gain an optional `model` (additive, per the compatibility rule), the engine selection path honours it against the chat role's own models from the Engines route, and only then the composer entry. Gate the UI on there being 2+ selectable models: the Engines route's `roles` is commonly empty on an unconfigured hub, and a picker with zero or one option renders nothing at all, never a disabled trigger. Acceptance: a turn sent with an explicit model runs on that model, proven by a test asserting the dispatch, not the payload; an unknown or unavailable model falls back to the role default with a status, never an error; the picker is absent with fewer than two models. Split out of RESP-04 on 2026-09-22 after Session A proved the backend gap. Out of scope: per-conversation persistence (PERSIST-CONV-01). Exit: `bash scripts/check.sh`.
...
- [ ] **RESP-01: the written register for the typed screen** (M). `persona.ts` gains a written policy beside the spoken one (full answers, structure where it helps, digits and formatting allowed, no "keep replies short"); the engagement fragment's brevity line moves into the spoken policy; `composePersonaPrompt` takes the surface class (`written` for chat and tv, `spoken` for robot, pod and phone voice, `glance` for overlay) and picks the policy; the dictation path marks the turn spoken. Acceptance: the seeded voice set does not regress; a written set of twenty typed corpus questions judged for completeness against the ChatGPT bar by the benches' judge; "shorter" and "tell me more" work on both surfaces. Design: [docs/plans/response-contract-by-surface-2026-09-21.md](plans/response-contract-by-surface-2026-09-21.md). Exit: `bash scripts/check.sh` and the named benches.
...
- [ ] **ADMIN-PERF-01: the admin's performance dashboard** (M, design note first, after DASH-CARDS-01). Objective: an owner/admin page, `/next/performance` (reached from Settings' Manage section and the System card), that shows how the hub is doing over time, from data the hub already records or the engines already report: per turn, the LM Studio-class numbers (model, thinking on or off, time to first token, total time, generation tokens per second, prompt and completion tokens, context used, the route taken) aggregated as medians and p95 per day and per model; the engines' health history and restarts; the memory and embedding queues (pending work, judge outcomes per day, the CHAT-06 ingestion counts); the label harvest's weekly report (guard hits, rule hits, the retire-eligible list from RVW-1b); disk (the data directory, backups, models) and RAM/CPU/GPU of the host as the hardware probe reports them; the Stack's job feed when a Stack is configured. Composed from the template's chart wrapper, data-tables and cards only, on the same card registry as DASH-CARDS-01 (each panel a card with a role floor of admin, so the admin can pick which ones sit on the home dashboard too). Backend: `GET /api/performance` (zod-openapi, admin only) reading the turn stats stored by SHELL-02 slice 5, the engines and repairs routes, the labels report and the probe; no new collection, only aggregation of what exists, with a stated retention (the turn stats table keeps 90 days). Acceptance: the page on the dev hub shows real numbers for today, every panel links to its source page, a non-admin gets 403 on the route and no rail or settings entry; captures at 1440 and 390. Out of scope: alerts (the notification system's own rules do that), exporting. Exit: `bash scripts/check.sh`.
...
- [ ] **ADMIN-COMPARE-01: tell whether a dumb reply is the model or the pipeline** (M)
    ...
    **Still open**: feature (a)'s frontend half (the "Compare with the bare model" action - a real kit-side gap, since the vendored `AssistantActionBar`'s own "More" menu has no extension slot today - and the canvas-split two-column compare view), feature (b) in full (the per-conversation `bare: true` switch wired into the real turn pipeline, a `bare` marker on the row, and the conversation history's own "bare model" badge on the message - COORDINATOR's own call: the memory judge does not extract from a bare-mode turn, since the reply isn't ours and the admin is testing, not talking).
