...
- [ ] **VOICE-LIVE-04: the composer shows a live waveform while dictating, as ChatGPT does** (S, Sonnet; owner's ask 2026-09-23, after VOICE-LIVE-02). Objective: while dictation records, the composer's text area gives way to a live bar waveform of the microphone (ChatGPT's dictation look), drawn in the browser from the same local level source as VOICE-LIVE-02 and sent nowhere; the bars stop and the transcript replaces them when recording ends. Prebuilt over hand-built: `react-audio-visualize`'s `LiveAudioVisualizer` (MIT, an `AnalyserNode` over a `MediaRecorder`, no network) through the package manager, its colors from the tokens; if the dictation adapter has no `MediaRecorder` to hand it, a `MediaRecorder` over the same stream is created for the visualizer only. Files: `frontend/src/lib/voice/sttDictationAdapter.ts`, the composer composition in `frontend/src/apps/chat/`. Mirror: the composer's existing dictation state. Acceptance: a headless run with the fake media stream shows moving bars during recording and the transcript after (captured), light and dark at 1440 and 390; the network log shows no request carrying audio. Exit: `bash scripts/check.sh` and the captures opened.
- [ ] **VOICE-LIVE-05: the voice mode looks like ChatGPT's, with a little flair** (S, Sonnet; owner's ask 2026-09-23, after VOICE-LIVE-02). Objective: the live voice session's centerpiece is an orb the size ChatGPT's is, breathing with the local level from VOICE-LIVE-02: the shipped Element's rings as they ship, restyled only through the tokens they read (a gradient and a soft glow per mode: listening, thinking, speaking, with the speaking color a token rather than the Element's hard-coded blue if that line is ours to override through a token, otherwise noted as an upstream ask), and one flair touch that costs nothing in latency (a slow hue drift while thinking, a ripple on the first spoken word), judged from a short recording, not a still. No hand-built orb: if the Element's rings cannot reach the look, the gap is named in the design record first and a maintained orb (a canvas library through the package manager, MIT, local only) is the next candidate, decided before a line is written. Acceptance: a recording of listen, think and speak on this Mac opened and judged by Jesse, plus the four captures; the level animation costs under 2 ms a frame (measured). Exit: `bash scripts/check.sh`, the recording and the captures.
...
- [ ] **CHAT-HEADER-02: the app's icon beside the header title, on every app page** (S, Sonnet; owner's ask 2026-09-23). Objective: the header's left slot (CHAT-HEADER-01's `HeaderExtraLeft`) shows the current app's icon before its title on every app page, the chat page included, drawn from the one definition the sidebar already reads (the package manifest's icon, the kit's icon set for the built-in pages), never a second icon table. Files: `frontend/src/apps/chat/chatHeaderData.tsx`, the header slot component, `frontend/src/next/NextRoutes.tsx`. Mirror: the sidebar's icon lookup. Acceptance: the chat header and one other app page show the same icon the sidebar shows for them, captured at 1440 and 390, light and dark; no icon is drawn by hand. Out of scope: the search control (SHELL-SEARCH-01). Exit: `bash scripts/check.sh` and the captures opened.
- [ ] **CHAT-HEADER-03: the title takes the header's free width** (S, Sonnet; owner's ask 2026-09-23). Objective: the conversation title in the header is clipped at about 290 px today ("why did corey feldmans friendshi"); it grows to the width the header has left after the sidebar trigger and the right-side controls, truncating with an ellipsis only when it must, the chevron staying beside the last visible character; on the phone the same rule at the small width. Files: the header slot component from CHAT-HEADER-01. Mirror: the template header's own flex rules (`min-w-0`, `flex-1`, `truncate`), tokens only. Acceptance: a 60-character title reads whole at 1440 and truncates with an ellipsis at 390, captured light and dark. Exit: `bash scripts/check.sh` and the captures opened.
