...
- [ ] **PERSONA-STEER-01: persona voice for written turns without spending prompt sentences on it** (L, design pass first, 2026-09-23; dev.md "The written prompt on tier 1, decided"). Objective: `WRITTEN_VOICE_PROSE = false` (`persona.ts`) took the written-adult prompt's persona and policy prose out entirely, because every measured shape carrying it on tier 1 cost 0.12x-0.48x of the bare floor while the two that carried none (the ceiling, arm e) cleared close to it - the design record's own table is this row's motivation. Change: a real prebuilt mechanism for varying the written reply's voice (formality, complexity, engagement, filler) that spends no prompt tokens doing it - the engine's own native activation steering (llama-server control vectors) or an equivalent, per org principle 6 ("Prebuilt over hand-built": a paragraph of personality prose instead of activation steering is hand-built, and loses to the prebuilt answer on the same terms). Needs its own design pass before a line is written: which dial(s) a control vector can actually steer on this model family, how a vector is trained or sourced, how `persona.ts`'s existing four-dial `Persona` shape maps onto it (a new field per dial, a single blended vector, dial values as vector-strength presets), and how it composes with `WRITTEN_VOICE_PROSE`'s own off switch (a real steering path presumably retires the constant rather than sitting beside it). Out of scope until the design pass names it: any specific vector format, training data, or llama-server API detail. Exit: a design note in dev.md naming the mechanism and the file/API surface, before any code.

- [ ] **WRITTEN-VOICE-TIER-01: WRITTEN_VOICE_PROSE becomes hardware-tier-aware** (S, 2026-09-23; dev.md "The written prompt on tier 1, decided"). Objective: `persona.ts`'s `WRITTEN_VOICE_PROSE` is a plain module constant (`false`), not a settings key, because tier 1's own 8B measured every voice-prose shape under the bar - a bigger model (tier 3, the Studio) may not have the same collapse, and this constant is the one place that decides whether the written class's own dial/policy prose (already declared, already tested) ever composes into a real prompt. Change: once STUDIO-ACCEPT-01 or an equivalent tier-3 workload measures the written voice twins (`FORMALITY_FRAGMENT_WRITTEN` and the rest, already exported) clearing WRITTEN-PARITY-01's own bar on that hardware, `WRITTEN_VOICE_PROSE` becomes a value the hardware probe proposes per tier (hardware-tiers-2026-09-23.md's own "the configuration is proposed, never fixed" pattern) rather than a hardcoded `false` - still no settings key, a probed default a person could override the same way any other tier default can be. Out of scope until tier 3 is measured: the probe wiring itself. Exit: the tier-3 measurement recorded in dev.md, then the constant's own change plus a test that tier 1 still defaults to off.
