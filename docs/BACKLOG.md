...
- [ ] **CHAT-LIST-01: a temporary-chat button beside New Thread** (S, Sonnet; owner's ask 2026-09-23). Objective: the thread list's header gains a second button next to New Thread that starts a temporary chat (TEMP-CHAT-01's flow, the same action the title chevron's "Start temporary chat" entry runs, which stays), composed from the same shipped parts as the New Thread button (the assistant-ui thread-list Element's new-thread primitive pattern and a kit icon button with the kit's incognito icon), tokens only. Files: `frontend/src/apps/chat/` (the thread list composition). Mirror: the New Thread button as it ships. Acceptance: the button appears beside New Thread, starts a temporary chat marked as such, a child profile sees exactly what TEMP-CHAT-01 allows it to see; captured at 1440 and 390. Exit: `bash scripts/check.sh` and the captures opened.
