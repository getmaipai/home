...
- [ ] **VOICE-LIVE-03: the chevron's voice selection and the wake-word shortcut** (S, Sonnet). Objective: the waveform's chevron lists the `tts` role's voices from the voice catalog, lists the microphones the browser reports (MediaDevices `enumerateDevices()` after permission, names shown without hardware ids, refreshed on `devicechange`) so a person with more than one input picks one, and, only when a wakeword package is installed, offers the shortcut to that device's declared wake-word setting (HANDSFREE-01 c's one key and its invariants). The microphone choice is a per-browser preference (the same class as a remembered tab), never a household setting; the dictation adapter and the live session open the chosen device. Built from the kit's own primitives (the menu the chevron already opens, a radio group); shadcn.io's "mic-selector" (the owner's reference, 2026-09-23; shadcn.io is a separate library not affiliated with shadcn/ui) is the shape to match, never a second library added. Not taken (owner's question, decided 2026-09-23): shadcn.io's "speech-input" button, because as it ships it recognizes speech through the browser's Web Speech API in Chrome and Edge, which sends the audio to the browser vendor's servers, an egress the privacy architecture forbids; its visual feedback (pulse rings while recording, a stop state, a loader) is what the shipped assistant-ui voice Element and the composer's waveform already do over the in-house `stt` role. Files: `composerVoiceControls.tsx`, `backend/src/lib/voiceCatalog.ts`, the settings registry. Mirror: the composer's thinking control menu (RESP-04 f). Acceptance: choosing a voice changes the next spoken reply; with two fake input devices the menu lists both and choosing the second makes the next dictation open that device id (asserted through the fake media stream); the shortcut is absent with no wakeword package and flips the one same setting when present (asserted). Out of scope: the wake word itself. Exit: `bash scripts/check.sh`.
