...
- [ ] **STUDIO-STAGE-01: the staging kit** (M, Sonnet). Objective: `scripts/studio/stage.sh` fetches every day-one artifact by its pinned URL and verifies its sha256 through the Stack's own download code (`stack/backend/src/lib/download.ts`), writing the Stack's store layout onto an external drive with a `STAGE.md` of every hash and size, and `--verify` re-hashes the drive; `--install <drive>` copies it into place. Items: llama-server b10797 macOS arm64, Qwen3.8-27B and Qwen3.6-35B-A3B as GGUF at Q6 at pinned revisions, the two Splash packages (17.4 GB and 20.9 GB, pre-seeded into the Hugging Face cache layout), the pins the installer and the Stack already carry. Acceptance: `--verify` green on this Mac's drive; a dry run proves whether `splash serve` honours the pre-seeded cache with the network denied, recorded either way; the Splash formula revision and its macOS 26.4 requirement in `STAGE.md`. Out of scope: any model not in MEASURE-02. Exit: `bash scripts/check.sh` and `STAGE.md`.
