...
- [ ] **REPLAY-BAR-01: the replay set's bars follow the adopted design, not the legacy corpus** (S, Sonnet, 2026-09-23). Objective: `control-negative-stephen-king` ("what's the latest Stephen king novel") expects `toolRan: null` from the tool-call corpus, but under the interim rule it is a world question with a fresh fact, so both paths "over-search" against the wrong bar; flip its expectation to `toolRan: "websearch"` with `outcomeArgsMatch` `expression` `stephen king` and `sourcesNonEmpty`. Same pass: `president-of-chile-when-born`'s turn 3 ("yes") moves out of that row into its own, `CONFIRM-01`, so the chile row's bar is turns 1 and 2. Files: `backend/scripts/bench/datasets/owner-replay.json`, `docs/dev.md` (one line per changed bar, with the reason). Acceptance: the fixture's own test passes; the next rerun's tables carry the new bars. Exit: `bash scripts/check.sh`. The same flip lands in commons' spec/llm/tool-call-corpus.json, where the Stephen King row still says expect_calls [] (Fable, 2026-09-23): spec first, one tag, then the pin.
