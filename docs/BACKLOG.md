...
- [ ] **SHELL-SEARCH-02: conversations, people, apps and settings join the header's one search** (M, Sonnet, 2026-09-23; design note `docs/plans/shell-search-2026-09-23.md`, which is the work order). Objective: one typed route `GET /api/search?q=` (`backend/src/routes/search.ts`, `createRoute` and Zod like `routes/approvals.ts`, in `/api/docs`) returning pointer results grouped by kind; one provider contract and registry (`backend/src/lib/search/providers.ts`) with four providers (conversations by title for the actor, temporary chats excluded; people by name through the existing disclosure filter; apps by manifest name; settings by the registry's labels); the kit's `HeaderSearch.tsx` gains one optional `remote` prop and renders the groups as `CommandGroup`/`CommandItem` (a ui tag and pin bump, the SHELL-SEARCH-01 pattern); Home supplies the fetch. Acceptance: a regression test per provider including the child-actor and temporary-chat cases; a kit test rendering remote groups; live on 8787 at 1440 and 390 with a seeded roster household, captures judged as one design. Out of scope: message-body search (SHELL-SEARCH-03, FTS5), in-app search, ranking, any spec record. Review medium (a route, a disclosure rule). Exit: `scripts/check.sh` as scoped, both repos.
