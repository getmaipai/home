...
- [ ] **CHAT-UI-01: five findings on /next/chat with the rail collapsed** (S). (1) Collapsed rail shows "..." or truncated section labels with a hover label: compare with the upstream demo's collapsed state; icon mode should hide group labels; fix kit-side or record the deviation. (2) The thread-list column keeps a gap on its left sized for the expanded rail: composition fix in NextChatPage. (3) The composer bounces at the bottom: two scroll containers; the thread viewport becomes the only scroller. (4) Collapse the thread-list column with a toggle, hover-to-peek as an overlay: exactly what the shipped `thread-list-sidebar` Element offers; a missing peek is a named gap, never hand-built. (5) A centered date and time divider after a long gap between turns: only if the Elements catalog has one; otherwise a named gap. Exit: `bash scripts/check.sh`, 1440 dark capture.
