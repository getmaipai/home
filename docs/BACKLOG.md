...
- [ ] **STUDIO-ACCEPT-01: one realistic acceptance workload** (M, Sonnet, after the Studio arrives; Astra's stack review). Two simultaneous conversations with speech on the shipping configuration: measure first useful answer, peak memory and cancellation per conversation, and set the concurrency and residency limits (the Stack's governor, the packages' warm limits and idle eviction) from the numbers, recorded in dev.md with the pinned set. Exit: the limits declared once and the workload green on them.
