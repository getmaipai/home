...
- [ ] **SIGNAL-02: a `computed` target, so arithmetic and the clock are never searched** (M, Sonnet, spec first, 2026-09-23; dev.md "U6 rerun ruling" (b) 3). Objective: "what's 12 plus 30" and "what time is it in Tokyo" carry `target: world`, so the interim rule forces a web search (9 to 24 s) for answers the context node already holds (`clock`) or the model computes. The signal vocabulary in `@maipai/spec` gains `computed` beside `world`, decided by the classifier from labelled rows (never a word rule); `isWorldQuestion` in `nodes/model.ts` forces search on `world` only; a `computed` question answers from context with no tools. Files: the spec's signal schema and fixtures (commons, tag bumped), the classifier's label set and its training rows, `nodes/model.ts`. Acceptance: both control rows clean 3/3 with totals within 1.25 times the old path's; no world-question row loses its search. Exit: `bash scripts/check.sh` and the rerun table.
