...
| LocalAI | MIT, upstream source. | One engine, OpenAI-compatible API, per-backend OCI images pulled on demand, a model gallery, Metal and MLX backends on macOS (surveyed 2026-09-17). | Passes on privacy. Its speech and image backends are not the chosen ones (Piper, diffusers), gallery provenance is theirs, OCI packaging fights launchd, and it has no pressure-based admission. Reject as platform; a legitimate `managedBy: "localai"` host. Source: [LocalAI](https://github.com/mudler/localai). |
| Harbor | Apache-2. | A CLI over Docker Compose wiring Ollama, llama.cpp, Open WebUI, Speaches, ComfyUI; host-native MLX on macOS (surveyed 2026-09-17). | A developer playground, Docker-first, no supervisor or governor. Reject as platform. Source: [Harbor](https://github.com/av/harbor). |
| mlx-serve | MIT, a ~7 MB Zig binary, no Python. | Text (MLX and every GGUF), FLUX.2 Klein, LTX-Video 2.5, Qwen3-TTS with cloning, ACE-Step, Hunyuan3D on one port with OpenAI, Anthropic and Ollama-shaped endpoints; launchd; resumable Hugging Face pulls (surveyed 2026-09-17). | Five of this document's roles in one process. Young (1.4k stars, one maintainer), macOS 26.2 or newer, no STT, no image editing, multi-model memory behavior undocumented. Adopt as a spawned engine candidate for the first-day bench beside llama-server and mlx-lm; Home (the Stack) keeps provenance and admission. Source: [mlx-serve](https://github.com/ddalcu/mlx-serve). |
| oMLX | Apache-2. | MLX language server: LLM, VLM, OCR, embed, rerank; continuous batching; RAM plus SSD paged KV cache; LRU eviction; per-model idle TTL; a process cap defaulting to RAM minus 8 GB; pin; `brew services`; menu-bar app (surveyed 2026-09-17). | Language only. Fills the "no supervisor" gap noted for `mlx-lm` above and its SSD prefix cache brings a long companion context back in seconds. The menu-bar app has Sparkle auto-update; the bare server does not. Adopt as the MLX language-engine candidate in place of a bare `mlx-lm` server, measured on the same bench. Source: [oMLX](https://github.com/jundot/omlx). |
...
Home keeps the guards, turn engine, consent and household provenance.
The memory governor, model identity, checksum records, engine
supervision and the stable URL contract are, as of 2026-09-17, the
design of MaiPai Stack (`getmaipai/stack`, `docs/dev.md`), the hub's
engine layer as its own product; until Home runs on it, the hub's own
supervisors carry the same rules. Home drops child
