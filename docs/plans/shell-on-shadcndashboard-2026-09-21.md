...
| Turn cost and timing (the stats) | cost-meter, context-breakdown, context-display, trace-waterfall, message-timing | turn stats from the backend. **Scoped 2026-09-22 (slice 5(d))**: `cost-meter` is **not applicable** - local inference has no per-turn price, and a self-hosted product has no number here for anyone to act on; dropped from the Details entry outright, not redefined into a "compute meter." `context-breakdown` is the one that survives long-term (per-category segments against the model's limit, what an admin actually reads) but `TurnStats` has no segment data today - `context-display` (a coarser total-only cousin, buildable from `prompt_tokens`/`predicted_tokens`/`cache_reuse_tokens`/`context_tokens` plus the model's own `measuredContextLength` from the Stack roles API) ships in its place for now; [CTX-SEG-01](https://github.com/getmaipai/home/issues/133) tracks the segment data, and `context-breakdown` retires `context-display` from Details the day it lands. `trace-waterfall` wants real per-phase spans (routing, a lookup, a tool call, generation) `TurnStats` doesn't carry either - [TRACE-SPANS-01](https://github.com/getmaipai/home/issues/134) tracks it (also wanted by ADMIN-PERF-01 and the bare-model compare tool's own trace column, not orphan work). `message-timing` ships now, unchanged - a flat stat row from `time_to_first_token_ms`/`total_time_ms`/`tokens_per_second`/`engine`, nothing missing. |
