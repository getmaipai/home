 ...
 * mechanism (`.style-calm`/`.style-studio`/the seven shadcn base-color
 * presets, ui/src/dashboard/css/globals.css) - a different mechanism,
 * the same setting, per the plan's own "Looks" ruling (docs/plans/
 * shell-on-shadcndashboard-2026-09-21.md: "ui.look keeps selecting
 * between them"). Resolution itself (`useLookValue`) is shared with the
 * old shell's `useLook` - only the effect that applies it differs. */
