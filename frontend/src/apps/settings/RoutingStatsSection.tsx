  ...
  return (
    <Section heading="Plugin routing">
      ...
      {error ? (
        ...
        <p className="text-base text-[var(--muted-foreground)]">No chat turns yet.</p>
      ) : (
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between text-base">
            ...
            <span className="text-[var(--muted-foreground)]">{stats.plugin}</span>
          </div>
          <div className="flex items-center justify-between text-base">
            <span>Matched a command</span>
            <span className="text-[var(--muted-foreground)]">{stats.command}</span>
          </div>
          <div className="flex items-center justify-between text-base">
            <span>Answered by the model</span>
            <span className="text-[var(--muted-foreground)]">{stats.model}</span>
          </div>
          {stats.pluginError > 0 ? (
            <div className="flex items-center justify-between text-base">
              <span>Matched a plugin but failed to run</span>
              <span className="text-[var(--muted-foreground)]">{stats.pluginError}</span>
            </div>
          ) : null}
          {stats.commandError > 0 ? (
            <div className="flex items-center justify-between text-base">
              <span>Matched a command but failed to run</span>
              <span className="text-[var(--muted-foreground)]">{stats.commandError}</span>
            </div>
          ) : null}
