 ...
 * itself uses). Ignores `delta`/`spoken_cue` entirely: a home card shows
 * the finished answer, not a live-typing effect or speech. */
export async function runFixedTurn(text: string): Promise<string> {
  ...
  for await (const event of readTurnStream(response)) {
    ...
    if (event.type !== "delta" && event.type !== "spoken_cue") {
