"use client";
/* eslint-disable jsx-a11y/img-redundant-alt -- K7 requires the exact accessible label requested by the chat contract. */
...
import { markCitations, type TurnWithMedia, type TurnWithSources } from "@/apps/chat/chatCitations";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <MessageError />
        <ChatSourceCaption />
        {media?.kind === "image" && sources?.[0] && <a href={sources[0].url} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer"><><img src={media.thumbnail ?? media.url} alt={`Picture from ${media.source}`} loading="lazy" className="max-w-full rounded-xl" /></></a>}
