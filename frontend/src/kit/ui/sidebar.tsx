...
function SidebarGroupAction({
  ...
  return (
    <Comp
      ...
      className={cn(
        "absolute top-3.5 right-3 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground ring-sidebar-ring outline-hidden transition-transform group-data-[collapsible=icon]:hidden after:absolute after:-inset-2 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 sm:after:hidden [&>svg]:size-4 [&>svg]:shrink-0",
        className
      )}
      {...props}
    />
...
// 48px targets, 16px text (docs/UI.md's floor): this is the shell's real
// nav, the sole way to reach every page on desktop/tablet, so the same
// floor step 1 applied to Button/Input/Select applies here too - even
// icon-collapsed (`group-data-[collapsible=icon]:size-12!`, not the
// generated default's `size-8!`).
const sidebarMenuButtonVariants = cva(
  // The generated version relies on the icon-collapsed box being too
  // small for any label text to fit (`[&>span:last-child]:truncate` plus
  // overflow-hidden) rather than actually hiding it - true at its own
  // 32px size, false once this file raises it to 48px for the
  // accessibility floor, which leaves just enough room for a clipped
  // sliver of the first letter to show through (found live, not by any
  // test - jsdom never lays anything out). `group-data-[collapsible=
  // icon]:[&>span:last-child]:hidden` hides it outright instead of
  // depending on that math staying accidentally true.
