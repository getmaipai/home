...
export default tseslint.config(
  ...
  },
  {
    ...
    files: ["**/*.{ts,tsx}"],
    ignores: ["src/apps/chat/thread.aui.tsx"],
    rules: {
  ...
  {
    // thread.aui.tsx (step 5b, ui-v0.2.0): the generic assistant-ui
    // wrappers it builds on moved to @maipai/ui, where the SAME
    // exemption class now lives (its own eslint.config.js override for
    // `src/assistant-ui/**/*.tsx`) - this file is the one piece that
    // stayed, since it mixes that vendored rendering with real Home
    // business logic (citations, memory, turn stats), and it still
    // carries the same vendored-registry nuances the kit's own copy
    // does: `aui-*`/`shimmer` CSS-module class names (its own
    // stylesheet, not a Tailwind utility) and the composer's own
    // intentional autofocus. Last in the array on purpose: it has to
    // come after the `src/apps/**/*.tsx` block above (same rule names,
    // same file also matched there) for these "off" settings to win.
    files: ["src/apps/chat/thread.aui.tsx"],
    rules: {
